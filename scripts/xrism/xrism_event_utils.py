...
def load_gti_intervals(gti_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    with opener(gti_path, "rb") as f:  # type: ignore[arg-type]
        ...
        # 条件分岐: `s_key is None or t_key is None` を満たす経路を評価する。
    ...
    # 条件分岐: `start.size != stop.size` を満たす経路を評価する。
...
def load_gti_intervals_from_event(event_path: Path) -> Tuple[np.ndarray, np.ndarray]:
    ...
    with opener(event_path, "rb") as f:  # type: ignore[arg-type]
        ...
        # 条件分岐: `"GTI" not in extname.upper()` を満たす経路を評価する。
...
def extract_pi_spectrum_from_event(
    ...
    *,
    ...
    with opener(event_path, "rb") as f:  # type: ignore[arg-type]
        ...
        # 条件分岐: `time_key is not None and gti_start is not None and gti_stop is not None` を満たす経路を評価する。
        if time_key is not None and gti_start is not None and gti_stop is not None:
            want.append(time_key)

        # 条件分岐: `pixel_key is not None and pixel_exclude` を満たす経路を評価する。
        ...
        for chunk in iter_bintable_column_chunks(f, layout=layout, columns=want, chunk_rows=int(chunk_rows)):
            ...
            m = np.isfinite(pi)
            # 条件分岐: `time_key is not None and time_key in chunk and gti_start is not None and gti_...` を満たす経路を評価する。
            ...
            # 条件分岐: `pixel_key is not None and pixel_key in chunk and pixel_exclude` を満たす経路を評価する。
            ...
            ok2 = idx >= 0
            # 条件分岐: `not np.any(ok2)` を満たす経路を評価する。
            if not np.any(ok2):
                continue
...
def compute_spectrum_diff_metrics(
    *,
    ...
    # 条件分岐: `ch.size != a.size or ch.size != b.size or e.size != ch.size` を満たす経路を評価する。
