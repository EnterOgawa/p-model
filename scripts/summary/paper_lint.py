...
- 図表インデックス（doc/paper/01_figures_index.md）に列挙された PNG が実在するか確認する
    ...
    *,
    ...
    for md_path in manuscript_paths:
        if not md_path.exists():
            ...
            continue
        used_keys |= _extract_used_citations(_read_text(md_path))
    ...
    for rel, caption in fig_items:
        ...
        if not png_path.exists():
            errors.append(f"missing figure file listed in index: {rel}")
            continue
        if not caption:
            warnings.append(f"no caption in figures index: {rel}")
    ...
    for md_path in manuscript_paths:
        if not md_path.exists():
            continue
        used_pngs |= _extract_png_refs(_read_text(md_path))
