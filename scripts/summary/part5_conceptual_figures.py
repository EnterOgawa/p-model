"""
Part V の一般読者向け概念図を生成する。

Inputs:
- このスクリプト内部で固定した概念図レイアウトと frozen な説明ラベル。
...
def _finalize_figure(fig: Any, *, paths: dict[str, Path]) -> dict[str, Any]:
    paths["private_pdf"].parent.mkdir(parents=True, exist_ok=True)
    with plt.rc_context({"savefig.bbox": "tight", "savefig.pad_inches": 0.03}):
...
def _draw_eht(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="ブラックホール影の差分", height_in=4.45)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ...
    ax.text(0.26, 0.80, "一般相対論", ha="center", va="center", fontsize=sizes["title"])
    ax.text(0.26, 0.64, "小さい影", ha="center", va="center", fontsize=sizes["note"])
    ax.text(0.62, 0.80, "P-model", ha="center", va="center", fontsize=sizes["title"])
    ax.text(0.62, 0.64, "約5%大きい影", ha="center", va="center", fontsize=sizes["note"])

    ax.add_patch(FancyArrowPatch((0.39, 0.64), (0.49, 0.64), arrowstyle="<->", mutation_scale=14, lw=1.5, color=OBS_COLOR))
    ax.text(0.44, 0.69, "4.8084%差", ha="center", va="bottom", fontsize=sizes["note"])

    ax.text(0.10, 0.33, "現在の誤差", ha="left", va="center", fontsize=sizes["axis"])
    ax.add_patch(Rectangle((0.28, 0.305), 0.28, 0.05, fc=WATCH_COLOR, ec=WATCH_COLOR, alpha=0.6))
    ax.text(0.58, 0.33, "差より大きい", ha="left", va="center", fontsize=sizes["note"])

    ax.text(0.10, 0.22, "ngEHT の誤差", ha="left", va="center", fontsize=sizes["axis"])
    ax.add_patch(Rectangle((0.28, 0.195), 0.09, 0.05, fc=PASS_COLOR, ec=PASS_COLOR, alpha=0.7))
    ax.text(0.39, 0.22, "差を分離できる見込み", ha="left", va="center", fontsize=sizes["note"])

    ax.text(0.50, 0.08, "影の直径差を直接測る観測が決定打になる。", ha="center", va="center", fontsize=sizes["note"])
...
def _draw_gw(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    panels = [
        ...
        (axes[1], "検出器3台（将来）", [("H1", 0.20, 0.72), ("L1", 0.76, 0.30), ("V1", 0.48, 0.82)], "3方向の情報で\n偏光を判別できる"),
    ]
...
def _draw_cosmo(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="宇宙論の独自項", template="paper_single_panel", height_in=4.65)
    z = np.linspace(0.0, 3.2, 240)
    lcdm = 0.22 + 0.52 * np.log1p(z)
    pmodel = lcdm + 0.045 * np.clip(z - 1.2, 0.0, None) ** 1.3
    ax.plot(z, lcdm, color=GR_COLOR, lw=2.2, label="標準理論")
    ...
    ax.text(2.35, 1.05, "高赤方偏移で差が開く", fontsize=sizes["note"], ha="center")
    ax.set_xlabel("赤方偏移 z", fontsize=sizes["axis"])
    ax.set_ylabel("距離指標の模式量", fontsize=sizes["axis"])
...
def _draw_sparc(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="銀河回転曲線の差分", template="paper_single_panel", height_in=4.65)
    ...
    ax.plot(r, observed, color=OBS_COLOR, lw=2.2, label="観測")
    ax.plot(r, baryon, color=GR_COLOR, lw=2.0, ls="--", label="見える物質だけ")
    ax.plot(r, pmodel, color=PASS_COLOR, lw=2.2, label="P-model")
    ax.text(10.2, 135.0, "差の説明を\nダークマター以外に置く", fontsize=sizes["note"], ha="left")
    ax.set_xlabel("銀河中心からの距離", fontsize=sizes["axis"])
    ax.set_ylabel("回転速度", fontsize=sizes["axis"])
...
def _draw_macro(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    fig.suptitle("マクロ量子干渉の破れ", fontsize=get_wavep_font_size("suptitle", name="part5_future_predictions"), y=0.985)
    sizes = {
        "title": get_wavep_font_size("title", name="part5_future_predictions"),
        "axis": get_wavep_font_size("axis", name="part5_future_predictions"),
        "tick": get_wavep_font_size("tick", name="part5_future_predictions"),
    ...
    top.set_ylim(0, 1)
    top.axis("off")
    top.text(0.50, 0.93, "同じ干渉縞が質量とともに崩れる", ha="center", va="top", fontsize=sizes["title"], transform=top.transAxes)
    top.add_patch(
        FancyBboxPatch(
            (0.07, 0.18),
            0.86,
    ...
    top.text(0.18, 0.10, "質量が小さい：くっきり見える", ha="center", fontsize=sizes["note"])
    top.text(0.80, 0.10, "質量が大きい：ぼやけて消える", ha="center", fontsize=sizes["note"])
    ...
    bottom.set_xticklabels(["原子", "大きな分子", "巨大分子\n(MAQRO級)"], fontsize=sizes["tick"])
...
def _draw_qc(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="量子計算のノイズ下限", template="paper_single_panel", height_in=4.65)
    ...
    floor = 1.0
    ax.plot(years, error, color=GR_COLOR, lw=1.8, marker="o", label="公開ベンチマークの改善")
    ax.axhline(floor, color=PMODEL_COLOR, lw=2.0, ls="--", label="P-model の下限ライン")
    ax.fill_between(years, floor, error, where=error >= floor, color="#E7EEF8", alpha=0.35)
    ax.text(2029.6, 1.14, "ここに届くかが焦点", fontsize=sizes["note"], ha="left", color=PMODEL_COLOR)
    ax.set_xlabel("年", fontsize=sizes["axis"])
    ax.set_ylabel("ゲート誤り率の模式量", fontsize=sizes["axis"])
...
def _draw_gq(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="弱場重力量子の差分", template="paper_single_panel", height_in=4.65)
    x = np.linspace(0.0, 1.0, 200)
    baseline = 0.35 + 0.25 * x
    pmodel = baseline + 0.035
    ax.plot(x, baseline, color=GR_COLOR, lw=2.0, label="標準理論")
    ax.plot(x, pmodel, color=PMODEL_COLOR, lw=2.0, ls="--", label="P-model")
    ax.fill_between(x, baseline - 0.10, baseline + 0.10, color=REFERENCE_COLOR, alpha=0.18, label="現在の精度")
    ax.fill_between(x, baseline - 0.025, baseline + 0.025, color=PASS_COLOR, alpha=0.20, label="将来の精度")
    arrow_x = 0.82
    ...
    ax.set_xlabel("観測配置の模式座標", fontsize=sizes["axis"])
    ax.set_ylabel("位相・周波数差の模式量", fontsize=sizes["axis"])
    ax.legend(loc="upper left", fontsize=sizes["legend"], frameon=False)
    ax.grid(alpha=0.22, linestyle="--")
    return _finalize_figure(fig, paths=paths)
...
def _draw_ddr(paths: dict[str, Path]) -> dict[str, Any]:
    fig, ax, sizes = _base_figure(title="距離指標の前提を切り分ける", template="paper_flowchart", height_in=4.55)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    ...
    ax.text(current_x + current_w / 2.0, 0.84, "現在", ha="center", fontsize=sizes["title"])
    ax.text(current_x + current_w / 2.0, 0.79, "前提が積み上がる測定", ha="center", fontsize=sizes["note"])
    ...
    ax.text(current_x + current_w / 2.0, 0.13, "前提が4段積み", ha="center", fontsize=sizes["note"])
    ...
    ax.text(future_x + future_w / 2.0, 0.84, "将来", ha="center", fontsize=sizes["title"])
    ax.text(future_x + future_w / 2.0, 0.79, "前提を減らした測定", ha="center", fontsize=sizes["note"])
    ax.text(future_x + future_w / 2.0, future_y + future_h / 2.0, "重力波\n（幾何距離）", ha="center", va="center", fontsize=sizes["axis"])
    ax.text(future_x + future_w / 2.0, 0.13, "前提が1段", ha="center", fontsize=sizes["note"])
    ...
    ax.text(0.535, 0.55, "段を減らして再測定", ha="center", fontsize=sizes["note"])
    ...
    ax.text(0.29, 0.065, "どの段がズレの原因か分かりにくい", ha="center", va="center", fontsize=sizes["note"])
    ...
    ax.text(0.75, 0.065, "段を減らせば原因を特定しやすい", ha="center", va="center", fontsize=sizes["note"])
    return _finalize_figure(fig, paths=paths)
