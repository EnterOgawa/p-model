...
def _extract_eht_requirements(eht: Dict[str, Any], *, kappa_budget: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for r in eht.get("rows") if isinstance(eht.get("rows"), list) else []:
        ...
        # 条件分岐: `key == "sgra" and isinstance(kappa_budget, dict)` を満たす経路を評価する。
        ...
        # 条件分岐: `math.isfinite(sigma_obs_now_with_kappa_kerr)` を満たす経路を評価する。
        if math.isfinite(sigma_obs_now_with_kappa_kerr):
            kappa_systematics_source = "kerr"

        # 条件分岐: `math.isfinite(sigma_obs_now_with_kappa_obs)` を満たす経路を評価する。
        ...
        # 条件分岐: `math.isfinite(sigma_obs_now_with_kappa_budget)` を満たす経路を評価する。
        ...
        # 条件分岐: `not math.isfinite(sigma_obs_now_with_kappa)` を満たす経路を評価する。
...
def _render_figure(payload: Dict[str, Any], *, out_png: Path) -> None:
    ...
    # 条件分岐: `isinstance(ratio, (int, float)) and isinstance(diff_pct, (int, float)) and ma...` を満たす経路を評価する。
