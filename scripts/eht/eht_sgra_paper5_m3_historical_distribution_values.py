...
def _parse_n_mu_sigma_from_wielgus_raw(raw: str) -> Optional[Tuple[int, float, float]]:
    ...
    # 条件分岐: `not (mu > 0.0 and sig >= 0.0)` を満たす経路を評価する。
...
def _best_assignment_by_count(clusters: Sequence[Sequence[Any]], expected_counts_by_key: Dict[str, int]) -> Optional[Dict[int, str]]:
    ...
    # 条件分岐: `len(clusters) != len(keys)` を満たす経路を評価する。
    ...
    for perm in itertools.permutations(keys):
        ...
        # 条件分岐: `best_cost is None or cost < best_cost` を満たす経路を評価する。

        if best_cost is None or cost < best_cost:
            best_cost = cost
            best_perm = perm

    # 条件分岐: `best_perm is None` を満たす経路を評価する。
...
def _calibrate_affine_from_mean_std(y_vals: Sequence[float], mu_target: float, sig_target: float) -> Optional[Tuple[float, float]]:
    ...
    my = sum(ys) / len(ys)
    sy = _std_pop(ys)
    # 条件分岐: `not (sy > 0.0 and mu_target > 0.0 and sig_target > 0.0)` を満たす経路を評価する。
...
def _extract_largest_rgb_image_from_pdf(pdf_path: Path) -> Optional["Any"]:
    ...
    try:
        ...
        for name, obj in xobj.items():
            ...
            area = w * h
            # 条件分岐: `best is None or (best_area is not None and area > best_area)` を満たす経路を評価する。
...
def _digitize_red_curve_from_bower2018_timeseries_pdf(
    ...
    *,
    ...
    # 条件分岐: `not (duration_h > 0.0 and n_points > 0 and mu_target > 0.0 and sig_target > 0.0)` を満たす経路を評価する。
    ...
    # 条件分岐: `len(y_lines) < 2 or len(x_lines) < 2` を満たす経路を評価する。
    ...
    # 条件分岐: `int(red.sum()) == 0` を満たす経路を評価する。
    if int(red.sum()) == 0:
    ...
    for x in sorted(set(int(v) for v in xs.tolist())):
        yy = ys[xs == x]
        # 条件分岐: `yy.size == 0` を満たす経路を評価する。
        if yy.size == 0:
            continue

        y_med_by_x[int(x)] = float(np.median(yy))

    # 条件分岐: `len(y_med_by_x) < 5` を満たす経路を評価する。
    ...
    # 条件分岐: `not (x_max > x_min)` を満たす経路を評価する。
    if not (x_max > x_min):
    ...
    for sx in sample_xs:
        ...
        # 条件分岐: `j <= 0` を満たす経路を評価する。
        if j <= 0:
            x_sel = x_keys[0]
        # 条件分岐: 前段条件が不成立で、`j >= len(x_keys)` を追加評価する。
        elif j >= len(x_keys):
            x_sel = x_keys[-1]
        else:
            left = x_keys_arr[j - 1]
...
def _digitize_red_markers_from_bower2018_timeseries_pdf_vector(
    ...
    *,
    ...
    for operands, op in cs.operations:
        ...
        # 条件分岐: `op == b"cm" and len(operands) == 6` を満たす経路を評価する。
        ...
        # 条件分岐: `op == b"n"` を満たす経路を評価する。

        if op == b"n":
            cur = []
            continue

    # 条件分岐: `len(centers) < int(n_points)` を満たす経路を評価する。
    ...
    try:
        with gzip.open(ps_gz_path, "rt", encoding="latin1", errors="ignore") as f:
            for line in f:
                ...
                if not in_block:
                    # 条件分岐: `line.startswith("%%BeginDocument:")` を満たす経路を評価する。
                ...
                buf.append(line)
                # 条件分岐: `line.startswith("%%EndDocument")` を満たす経路を評価する。
                if line.startswith("%%EndDocument"):
                    block = buf
                    in_block = False
                    buf = []
                    text = "".join(block)
                    # 条件分岐: `all(s in text for s in want)` を満たす経路を評価する。
...
def _parse_marrone2006_sma_from_thesis_ps_gz(
    ...
    # 条件分岐: `block_lines is None` を満たす経路を評価する。
    ...
    for lt, pts in pts_by_lt.items():
        ...
        # 条件分岐: `not lab` を満たす経路を評価する。
        if not lab:
            continue

        ymd = _month_day_to_ymd_2005(lab)
        # 条件分岐: `not ymd` を満たす経路を評価する。
        if not ymd:
            continue
    ...
    for ymd, meta in expected_by_date.items():
        ...
        # 条件分岐: `not (dur_h > 0.0 and mu > 0.0 and sig > 0.0)` を満たす経路を評価する。
        ...
        # 条件分岐: `exp_n and exp_n != len(pts)` を満たす経路を評価する。
...
def _parse_yusef2009_sma_fig11_ps(
    ...
    notes: List[str] = []
    # 条件分岐: `not ps_path.exists()` を満たす経路を評価する。
    if not ps_path.exists():
        return {}, ["missing_file"]

    # 条件分岐: `not expected_by_date` を満たす経路を評価する。
    ...
    # 条件分岐: `assignment is None` を満たす経路を評価する。
    ...
    for i, cluster in enumerate(clusters):
        ...
        out[str(ymd)] = rows
        # 条件分岐: `int(meta.get("n_points") or 0) != len(cluster)` を満たす経路を評価する。
...
def _parse_witzel2021_dbf1(path: Path) -> Dict[Tuple[str, float], List[Tuple[float, float]]]:
    ...
    for raw in path.read_text(encoding="utf-8", errors="replace").splitlines():
        # 条件分岐: `not (raw.startswith("SMA") or raw.startswith("ALMA"))` を満たす経路を評価する。
...
def _parse_fazio2018_dbf3_sma(path: Path) -> Dict[str, List[Tuple[float, float]]]:
    ...
    for raw in path.read_text(encoding="utf-8", errors="replace").splitlines():
        # 条件分岐: `not raw.startswith("SMA")` を満たす経路を評価する。
...
def _extract_idl_panels(text: str) -> List[Tuple[float, float, float, float]]:
    ...
    for (x0, y0), wh in hv.items():
        # 条件分岐: `wh.get("w") is None or wh.get("h") is None` を満たす経路を評価する。
...
def _extract_idl_ticks(lines: Sequence[str]) -> List[Tuple[float, float, float]]:
    ...
    for raw in lines:
        ...
        # 条件分岐: `m2 and cur_xy is not None` を満たす経路を評価する。
...
def _fit_panel_maps(lines: Sequence[str], panels: Sequence[Tuple[float, float, float, float]]) -> List[_PanelMap]:
    ...
    for x0, y0, w, h in panels:
        ...
        # 条件分岐: `len(xt) < 2` を満たす経路を評価する。
        if len(xt) < 2:
            continue

        y_map = y_map_by_row.get(float(y0))
        # 条件分岐: `y_map is None` を満たす経路を評価する。
        if y_map is None:
            continue
...
def _parse_gnuplot_eps_ticks(lines: Sequence[str]) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]]]:
    ...
    for raw in lines:
        ...
        # 条件分岐: `"MCshow" in raw and cur_xy is not None and cur_val is not None` を満たす経路を評価する。
        ...
        # 条件分岐: `"MRshow" in raw and cur_xy is not None and cur_val is not None` を満たす経路を評価する。
...
def _parse_marrone2008_f2b_sgra(path: Path) -> List[Tuple[float, float]]:
    ...
    # 条件分岐: `x_map is None or y_map is None` を満たす経路を評価する。
    ...
    for raw in lines:
        ...
        x = float(m.group("x"))
        y = float(m.group("y"))
        # 条件分岐: `x0 is not None and x1 is not None and not (x0 <= x <= x1)` を満たす経路を評価する。
        if x0 is not None and x1 is not None and not (x0 <= x <= x1):
            continue

        # 条件分岐: `y0 is not None and y1 is not None and not (y0 <= y <= y1)` を満たす経路を評価する。
...
def main() -> int:
    ...
    # 条件分岐: `not in_w.exists()` を満たす経路を評価する。
    if not in_w.exists():
    ...
    # 条件分岐: `args.mode == "pre_eht_2017_apr11"` を満たす経路を評価する。
    if args.mode == "pre_eht_2017_apr11":
        cand = w_der.get("paper5_m3_historical_distribution_candidate_pre_eht_2017_apr11") or {}
    else:
        cand = {}

    # 条件分岐: `not (isinstance(cand, dict) and bool(cand.get("ok")) and isinstance(cand.get(...` を満たす経路を評価する。
    ...
    sample_2017_vals = []
    # 条件分岐: `isinstance(sample_2017, dict) and isinstance(sample_2017.get("samples"), list)` を満たす経路を評価する。
    ...
    # 条件分岐: `dex_eps.exists()` を満たす経路を評価する。
    if dex_eps.exists():
    ...
    # 条件分岐: `w_dbf1.exists()` を満たす経路を評価する。
    if w_dbf1.exists():
    ...
    # 条件分岐: `f_dbf3.exists()` を満たす経路を評価する。
    if f_dbf3.exists():
    ...
    for c in curves:
        # 条件分岐: `not (isinstance(c, dict) and str(c.get("reference_key") or "") == "Yusef2009")` を満たす経路を評価する。
        ...
        # 条件分岐: `parsed is None` を満たす経路を評価する。
        if parsed is None:
            continue
    ...
    # 条件分岐: 前段条件が不成立で、`yusef_expected and not yusef_ps.exists()` を追加評価する。
    ...
    for c in curves:
        # 条件分岐: `not (isinstance(c, dict) and str(c.get("reference_key") or "") == "Marrone2006")` を満たす経路を評価する。
    ...
    # 条件分岐: `marrone_ps_gz.exists() and marrone_expected` を満たす経路を評価する。
    ...
    # 条件分岐: 前段条件が不成立で、`marrone_expected and not marrone_ps_gz.exists()` を追加評価する。
    ...
    for c in curves:
        # 条件分岐: `not (isinstance(c, dict) and str(c.get("reference_key") or "") == "Bower2018")` を満たす経路を評価する。
        if not (isinstance(c, dict) and str(c.get("reference_key") or "") == "Bower2018"):
            continue

        ymd = str(c.get("date_ymd") or "")
        # 条件分岐: `ymd not in bower_pdf_by_date` を満たす経路を評価する。
    ...
    for c in curves:
        ...
        # 条件分岐: `ref == "Dexter2014"` を満たす経路を評価する。
        if ref == "Dexter2014":
            rows = dex_by_date.get(ymd)
            method = "Dexter2014_IDL_EPS"
            # 条件分岐: `rows is None` を満たす経路を評価する。
            if rows is None:
                notes.append("date_not_found_in_eps")

        # 条件分岐: 前段条件が不成立で、`ref == "Witzel2021"` を追加評価する。
        elif ref == "Witzel2021":
            ...
            # 条件分岐: 前段条件が不成立で、`arr == "SMA"` を追加評価する。
            elif arr == "SMA":
        ...
        # 条件分岐: 前段条件が不成立で、`ref == "fazio:2018"` を追加評価する。
        elif ref == "fazio:2018":
            # 条件分岐: `arr == "SMA"` を満たす経路を評価する。
            if arr == "SMA":
        ...
        # 条件分岐: 前段条件が不成立で、`ref == "Yusef2009"` を追加評価する。
        elif ref == "Yusef2009":
            # 条件分岐: `arr == "SMA"` を満たす経路を評価する。
            if arr == "SMA":
                rows = yusef_by_date.get(ymd)
                method = "Yusef2009_PGPLOT_PS_fig11"
                # 条件分岐: `yusef_notes` を満たす経路を評価する。
                if yusef_notes:
        ...
        # 条件分岐: 前段条件が不成立で、`ref == "Bower2018"` を追加評価する。
        elif ref == "Bower2018":
            # 条件分岐: `arr == "ALMA"` を満たす経路を評価する。
            if arr == "ALMA":
                rows = bower_by_date.get(ymd)
                method = "Bower2018_digitized_raster_PDF_timeseries"
                # 条件分岐: `ymd in bower_notes` を満たす経路を評価する。
        ...
        # 条件分岐: `ref == "Marrone2008" and rows is None` を満たす経路を評価する。
        ...
        # 条件分岐: `isinstance(rows, list) and len(rows) == 0` を満たす経路を評価する。
    ...
    for (ref, arr, ymd), (method, rows, notes) in sorted(time_series_by_curve.items()):
        seg_n = 0
        for c in curves:
            # 条件分岐: `isinstance(c, dict) and str(c.get("reference_key") or "") == ref and str(c.ge...` を満たす経路を評価する。
        ...
        for s in segs:
            ...
            # 条件分岐: `row.get("ok") and isinstance(row.get("mi3"), (int, float))` を満たす経路を評価する。
    ...
    # 条件分岐: `plot_err is None` を満たす経路を評価する。
    if plot_err is None:
