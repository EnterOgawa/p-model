...
def _download(url: str, dst: Path, *, force: bool) -> None:
    ...
    print(f"[dl] {url}")
    req = urllib.request.Request(url, headers={"User-Agent": "waveP-gravity-sgra-fetch/1.0"})
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Fetch GRAVITY Collaboration (2020) 'The flux distribution of Sgr A*' (PDF + arXiv source)."
    )
    ...
    row: Dict[str, Any] = {
        "key": "gravity_sgra_flux_distribution_2020",
        "arxiv": arxiv_id,
        "label": "GRAVITY Collaboration (2020) The flux distribution of Sgr A*",
    ...
    manifest = {
        ...
        "output": "output/private/eht/gravity_sgra_flux_sources_manifest.json",
        "rows": [row],
    }
    out_json = out_dir / "gravity_sgra_flux_sources_manifest.json"
        ...
        worklog.append_event(
            {
                ...
                "topic": "eht",
                "action": "fetch_gravity_sgra_flux_paper",
                "outputs": [str(out_json.relative_to(root)).replace("\\", "/")],
                "metrics": {
                    "papers": 1,
                    "pdf_ok": 1 if row.get("local_pdf_sha256") else 0,
                    "src_ok": 1 if row.get("local_src_sha256") else 0,
                    "extract_ok": 1 if row.get("extract_ok") is True else 0,
                },
            }
        )
    except Exception:
