...
# -*- coding: utf-8 -*-
"""
mass_origin_geometric_binding_selection_third_retry.py

Step 8.7.55.2.416:
Reinject the reflective-cavity third retry result into the post-linearized
...
Outputs:
  ...
  - output/public/quantum/mass_origin_geometric_binding_selection_third_retry_rows.csv
"""
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_geometric_binding_selection_third_retry_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_geometric_binding_selection_third_retry_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.416"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retry geometric binding-route selection after reflective-cavity third retry.")
...
def _build_rows(
    *,
    ...
    return [
        {
            "row_id": "geometric_binding_selection_third_retry_complete",
            "status": "pass",
            "metric": "geometric binding selection third retry complete",
            "value": 1.0,
            "note": "This step reinjects the reflective-cavity third retry into the post-linearized binding selection gate.",
        },
        {
            "row_id": "selected_binding_route_unique",
            "status": "pass" if selected_binding_route_or_none is not None else "reject",
            "metric": "selected geometric binding route unique after third retry",
        ...
        },
        {
            ...
            "metric": "discrete-spectrum reopen ready after geometric third retry",
        ...
        },
        {
            ...
            "metric": "remaining binding blocker count after geometric third retry",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    for path in (ROUTE_CONTRACT_JSON, THIRD_RETRY_JSON):
        _require_path(path)
    ...
    geometric_boundary_promotion_rule_available = bool(third_retry_summary.get("geometric_boundary_promotion_rule_available", False))
    ...
    remaining_binding_blockers = [] if geometric_boundary_promotion_rule_available else ["shell_quantization_domain_statement"]
    selection_third_retry_nonclosure_reason_or_none = None if geometric_boundary_promotion_rule_available else "shell_quantization_reflective_cavity_rule_still_unavailable"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "geometric binding selection third retry",
        },
        "inputs": {
        ...
        "intent": "Decide whether the geometric reflective-boundary route can now be selected as the unique post-linearized reopening channel after the third retry.",
        "formulas": {
            ...
            "current_absence": "the reflective cavity rule remains unavailable because the shell-quantization domain statement is still not public canonical",
        },
        "rows": rows,
        "summary": {
            ...
            "remaining_binding_blockers": remaining_binding_blockers,
            "selection_third_retry_nonclosure_reason_or_none": selection_third_retry_nonclosure_reason_or_none,
        },
        "decision": {
            "overall_status": "geometric_binding_selection_third_retry_frozen",
