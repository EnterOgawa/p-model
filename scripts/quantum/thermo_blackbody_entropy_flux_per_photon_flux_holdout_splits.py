...
def main() -> None:
    ...
    ratio_entropy_flux_per_photon_flux = entropy_flux / np.maximum(1e-300, photon_flux)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_entropy_flux_per_photon_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_entropy_flux_per_photon_flux, train_idx)
        ratio_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_entropy_flux_per_photon_flux,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_entropy_flux_per_photon_flux, train_idx, exponent=0.0)
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_flux_per_photon_flux_holdout_splits.csv"
    ...
    ax.set_title("Blackbody ratio holdout: Phi_s/Phi_n = const")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_flux_per_photon_flux_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_entropy_flux_per_photon_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for Phi_s/Phi_n based on blackbody radiation identities.",
