"""
目的: 黒体 holdout 監査の heat capacity flux 指標図を再生成する。
...
def main() -> None:
    ...
    heat_capacity_flux_coeff = (speed_of_light / 4.0) * heat_capacity_coeff

    temperature_grid_k = np.logspace(0.0, 4.0, 80)
    heat_capacity_flux = heat_capacity_flux_coeff * (temperature_grid_k**3)

    heat_capacity_flux_sigma = np.maximum(1e-12, 1e-6 * heat_capacity_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, heat_capacity_flux, train_idx)
        heat_capacity_flux_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=heat_capacity_flux,
            y_pred=heat_capacity_flux_pred,
            sigma=heat_capacity_flux_sigma,
            param_count=2,
            is_train=True,
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, heat_capacity_flux, train_idx, exponent=3.0)
        heat_capacity_flux_pred_fixed = coeff_fixed * (temperature_grid_k**3.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=heat_capacity_flux,
            y_pred=heat_capacity_flux_pred_fixed,
            sigma=heat_capacity_flux_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_heat_capacity_flux_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody heat-capacity flux holdout: Phi_cV(T)=(c/4)c_v(T)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_heat_capacity_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_heat_capacity_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "heat_capacity_flux_coeff": heat_capacity_flux_coeff,
                },
                "assumptions": {
                ...
                "notes": [
                    "Analytic holdout for blackbody heat-capacity flux Phi_cV(T)=(c/4)c_v(T).",
