...
class ConstantSpec:
    code: str
    # NIST Cuu constant code (e.g., mp, mn, md, rd).
    url: str
    relpath: str
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Fetch NIST Cuu CODATA constants used as nuclear baseline constraints for deuteron binding (mp,mn,md,rd). "
            "Writes a manifest.json and extracted_values.json under data/quantum/sources/."
        )
    )
    ap.add_argument("--offline", action="store_true", help="Do not download; only verify expected files exist.")
    ap.add_argument(
        "--out-dirname",
        default="nist_codata_2022_nuclear_baseline",
        help=(
            "Output directory name under data/quantum/sources/. "
            "Default keeps the historical path for deuteron-only baseline."
        ),
    )
    ap.add_argument(
        "--include-light-nuclei",
        action="store_true",
        help="Also include light-nuclei masses/radii (mt,mh,mal,ral,rp) for A-dependence baselines.",
    )
    ap.add_argument(
        "--expect-codata-year",
        type=int,
    ...
    # NIST Cuu constant pages (stable per-code URLs).
    base = "https://physics.nist.gov/cgi-bin/cuu/Value"
    specs: list[ConstantSpec] = [
        ConstantSpec(code="mp", url=f"{base}?mp", relpath="nist_cuu_Value_mp.html"),
        ConstantSpec(code="mn", url=f"{base}?mn", relpath="nist_cuu_Value_mn.html"),
    ...
    if args.include_light_nuclei:
        specs.extend(
            [
                ConstantSpec(code="rp", url=f"{base}?rp", relpath="nist_cuu_Value_rp.html"),
                ConstantSpec(code="mt", url=f"{base}?mt", relpath="nist_cuu_Value_mt.html"),
                ConstantSpec(code="mh", url=f"{base}?mh", relpath="nist_cuu_Value_mh.html"),
                ConstantSpec(code="mal", url=f"{base}?mal", relpath="nist_cuu_Value_mal.html"),
                ConstantSpec(code="ral", url=f"{base}?ral", relpath="nist_cuu_Value_ral.html"),
            ]
        )
    ...
    dataset_desc = (
        "NIST Cuu CODATA constants (mp,mn,md,rd) for nuclear baseline constraints"
        if not args.include_light_nuclei
        else "NIST Cuu CODATA constants (mp,mn,md,mt,mh,mal,rp,rd,ral) for light-nuclei nuclear baselines"
    )
    ...
    manifest["files"].append(
        {
            "url": None,
            "path": str(out_extracted),
            "bytes": int(out_extracted.stat().st_size),
            "sha256": _sha256(out_extracted),
            "derived_from": [spec.relpath for spec in specs],
        }
    )
