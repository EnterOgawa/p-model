"""
目的: 黒体 holdout 監査の momentum density 指標図を再生成する。
...
def main() -> None:
    ...
    momentum_sigma = np.maximum(1e-12, 1e-6 * momentum_density)  # 1 ppm relative tolerance
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, momentum_density, train_idx)
        momentum_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=momentum_density, y_pred=momentum_pred, sigma=momentum_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=momentum_density, y_pred=momentum_pred, sigma=momentum_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, momentum_density, train_idx, exponent=4.0)
        momentum_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=momentum_density,
            y_pred=momentum_pred_fixed,
            sigma=momentum_sigma,
            param_count=1,
            is_train=True,
        )
        ...
        models["fixed_exponent_4_fit"] = {
            "params": {"coeff": float(coeff_fixed), "exponent": 4.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_momentum_density_holdout_splits.csv"
    ...
    for sp in split_results:
        ...
        model_fixed = models.get("fixed_exponent_4_fit") if isinstance(models, dict) else None
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 4 (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody momentum density holdout: p(T)=u(T)/c")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_momentum_density_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_momentum_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody momentum density p(T)=u(T)/c=(a/c)T^4.",
