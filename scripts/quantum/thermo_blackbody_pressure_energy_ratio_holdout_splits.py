"""
目的: 黒体 holdout 監査の pressure energy ratio 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_pressure_energy = pressure_density / np.maximum(1e-300, energy_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_pressure_energy)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_pressure_energy, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_pressure_energy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_pressure_energy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_pressure_energy, train_idx, exponent=0.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k**0.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_pressure_energy,
    ...
    out_csv = out_dir / "thermo_blackbody_pressure_energy_ratio_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: P/u = 1/3")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_pressure_energy_ratio_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_pressure_energy_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for P/u = 1/3 based on blackbody radiation equation of state.",
