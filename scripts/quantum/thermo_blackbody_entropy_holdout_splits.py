...
def main() -> None:
    ...
    # 1 ppm relative tolerance (operational, to avoid zero sigma).
    entropy_sigma = np.maximum(1e-12, 1e-6 * entropy_density)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, entropy_density, train_idx)
        entropy_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=entropy_density, y_pred=entropy_pred, sigma=entropy_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=entropy_density, y_pred=entropy_pred, sigma=entropy_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, entropy_density, train_idx, exponent=3.0)
        entropy_pred_fixed = coeff_fixed * (temperature_grid_k**3.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=entropy_density,
            y_pred=entropy_pred_fixed,
            sigma=entropy_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_holdout_splits.csv"
    ...
    ax.set_title("Blackbody entropy holdout: s(T) scaling across temperature bands")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_entropy_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "This is an analytic thermo holdout: it verifies that s(T) scaling extrapolates across temperature bands under the fixed constants.",
                    "Because the dataset is formula-derived, the purpose is procedural (catching regressions/implementation errors), not observational inference.",
                ],
            },
            ensure_ascii=False,
            indent=2,
