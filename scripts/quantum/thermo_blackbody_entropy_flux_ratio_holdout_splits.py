"""
目的: 黒体 holdout 監査の entropy flux ratio 指標図を再生成する。
...
def main() -> None:
    ...
    flux_density = stefan_boltzmann_sigma * (temperature_grid_k**4)

    ratio_entropy_flux = entropy_density / np.maximum(1e-300, flux_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_entropy_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_entropy_flux, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_entropy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_entropy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_entropy_flux, train_idx, exponent=-1.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k ** -1.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_entropy_flux,
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_flux_ratio_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: s/M = (16/3c) T^-1")
    ax.set_title("Blackbody ratio holdout: s/M = (16/3c) T^-1")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_flux_ratio_holdout_splits.png"
