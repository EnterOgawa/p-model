...
def main() -> None:
    ...
    entropy_flux_sigma = np.maximum(1e-12, 1e-6 * entropy_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, entropy_flux, train_idx)
        entropy_flux_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=entropy_flux, y_pred=entropy_flux_pred, sigma=entropy_flux_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=entropy_flux, y_pred=entropy_flux_pred, sigma=entropy_flux_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, entropy_flux, train_idx, exponent=3.0)
        entropy_flux_pred_fixed = coeff_fixed * (temperature_grid_k**3.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=entropy_flux,
            y_pred=entropy_flux_pred_fixed,
            sigma=entropy_flux_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_flux_holdout_splits.csv"
    ...
    ax.set_title("Blackbody entropy flux holdout: J_s(T)=(c/4)s(T)")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_flux_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_entropy_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody entropy flux J_s(T)=(c/4)s(T)=(4/3)sigma*T^3.",
