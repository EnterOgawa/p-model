"""
目的: 黒体 holdout 監査の flux 指標図を再生成する。
...
def main() -> None:
    ...
    sigma_constant = _as_float(_get_constant(extracted, "sigma").get("value_si"))

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    flux_w_per_m2 = sigma_constant * (temperature_grid_k**4)

    flux_sigma = np.maximum(1e-12, 1e-6 * flux_w_per_m2)  # 1 ppm relative tolerance
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, flux_w_per_m2, train_idx)
        flux_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=flux_w_per_m2, y_pred=flux_pred, sigma=flux_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=flux_w_per_m2, y_pred=flux_pred, sigma=flux_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, flux_w_per_m2, train_idx, exponent=4.0)
        flux_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
    ...
    out_csv = out_dir / "thermo_blackbody_flux_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody flux holdout: M(T)=σT^4 scaling across temperature bands")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {"sigma_W_per_m2K4": sigma_constant},
                "assumptions": {
                ...
                "notes": [
                    "Analytic holdout for blackbody flux M(T)=σ T^4 using CODATA/NIST SI constants.",
