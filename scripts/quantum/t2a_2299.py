#!/usr/bin/env python3
"""Generate 8.7.56.2299-.2302 hybrid seventh farther registry artifacts."""
...
STEM = build_compact_artifact_stem(
    STEP_TAG,
    "harmonic_hybrid_s7_s8_farther_registry",
    prefix="q",
)

PRIOR_CLASS = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_retained_1966080_next"
)
BRANCH_CLASS_SEVENTH = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_retained_2064384_next"
)
BRANCH_CLASS_EIGHTH = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s8_promoted_2064384_next"
)
BRANCH_CLASS_RESET = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7s8_farther_exhausted_pack_update_next"
)
...
def build_formulae() -> dict[str, str]:
    ...
    return {
        "gate_a_rule": "Gate A retains the same seventh segment if the accelerated first shot survives through harmonic 2064384.",
        "gate_b_rule": "Gate B promotes the reserve eighth exact fallback if the same seventh segment fails and the reserve eighth passes farther holdout and monitor windows.",
        "gate_c_rule": "Gate C opens only if both the accelerated same-seventh route and the reserve eighth exact fallback fail on the same farther branch.",
    }
...
def main() -> None:
    """Execute the hybrid seventh farther registry refresh."""
    for path in (
        STATUS,
        ROADMAP,
        AI_CONTEXT,
    ...
    inventory_ready = bool(
        prior_summary["gate_a_same_seventh_piecewise_validation_to_2064384_retained"]
        or prior_summary["gate_b_eighth_piecewise_reactivation_selected"]
        or prior_summary["gate_c_substantive_pack_update_required"]
    )
    gate_a = bool(prior_summary["gate_a_same_seventh_piecewise_validation_to_2064384_retained"])
    ...
    rows = [
        ...
        sign_base.row(
            "gate_a_same_seventh_piecewise_validation_to_2064384_retained",
            "pass" if gate_a else "reject",
            "Gate A same seventh validation through harmonic 2064384 retained",
    ...
    summary = {
        ...
        "gate_a_same_seventh_piecewise_validation_to_2064384_retained": gate_a,
    ...
    declaration_payload = sign_base.payload(
        ...
        summary,
        {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_farther_registry_declared",
        ...
        },
        {
            ...
            "hits": {
                "status_branch_hit": find_line(status_text, "8.7.56.2299"),
                "roadmap_branch_hit": find_line(roadmap_text, ".2299-.2302"),
                "current_problem_hit": find_line(current_problem_text, "8.7.56.2299"),
                "current_status_hit": find_line(current_status_text, "8.7.56.2299"),
                "unified_roadmap_hit": find_line(unified_text, ".2299-.2302"),
                "long_roadmap_hit": find_line(long_text, ".2299-.2302"),
                "part5_hit": find_line(part5_text, ".2299-.2302"),
            },
        },
    )
    ...
    route_payload = {
        ...
        "summary": summary,
        "decision": {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_farther_registry_route_synced",
            "branch_completed": True,
            "next_required_artifacts": [next_route_name],
        },
