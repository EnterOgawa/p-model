...
def main() -> None:
    ...
    ratio_enthalpy_flux = enthalpy_density / np.maximum(1e-300, energy_flux)
    ratio_sigma = np.maximum(1e-12, 1e-6 * np.abs(ratio_enthalpy_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_enthalpy_flux, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_enthalpy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_enthalpy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_enthalpy_flux, train_idx, exponent=0.0)
        ratio_pred_fixed = np.abs(coeff_fixed_abs * (temperature_grid_k**0.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_enthalpy_flux,
    ...
    out_csv = out_dir / "thermo_blackbody_enthalpy_flux_ratio_holdout_splits.csv"
    ...
    ax.set_title("Blackbody ratio holdout: h/M = 16/(3c)")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_enthalpy_flux_ratio_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_enthalpy_flux_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "h/M is positive for photon gas; model fit is applied to abs(h/M) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for h/M = 16/(3c) based on blackbody enthalpy-density and energy-flux identities.",
