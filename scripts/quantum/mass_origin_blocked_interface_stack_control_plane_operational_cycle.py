...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_operational_cycle.py

Step 8.7.55.2.34:
Freeze one current-state operational-cycle artifact for the mass-origin
blocked interface-stack control-plane pack, so downstream readers can see
whether the control-plane continuity artifact and the control-plane rerun
policy agree on the present no-change-hold state.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_operational_cycle_rows.csv
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_operational_cycle.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_operational_cycle_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.34"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the operational-cycle state for the mass-origin blocked interface-stack control-plane pack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _extract_cycle_state(continuity: Dict[str, Any], rerun_policy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "control_plane_complete": bool(rerun_decision.get("control_plane_complete", False)),
        "keep_mass_origin_branch_blocked": bool(rerun_decision.get("keep_mass_origin_branch_blocked", True)),
        "terminal_control_plane_status": str(
            continuity_summary.get("terminal_control_plane_status", rerun_summary.get("terminal_control_plane_status", ""))
        ).strip(),
        "control_plane_signature_sha256": str(
            continuity_summary.get(
                "control_plane_signature_sha256",
                rerun_summary.get("control_plane_signature_sha256", ""),
            )
        ).strip().lower(),
...
def _build_rows(cycle_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    terminal_control_plane_status = str(cycle_state.get("terminal_control_plane_status", ""))
    ...
    return [
        ...
        },
        {
            ...
            "metric": "control-plane continuity state is unchanged",
            "value": 1.0 if not bool(cycle_state.get("continuity_state_changed", False)) else 0.0,
            "note": "The rerun chain should stay idle until the control-plane continuity state changes.",
        },
        {
            "row_id": "rerun_required_now_false",
            "status": "pass" if not bool(cycle_state.get("rerun_required_now", False)) else "watch",
            "metric": "control-plane rerun policy does not currently require the chain to run",
            "value": 1.0 if not bool(cycle_state.get("rerun_required_now", False)) else 0.0,
            "note": "This row is watch only when the policy already says the control-plane chain must run now.",
        },
        {
            "row_id": "rerun_trigger_is_control_plane_continuity_state_changed",
            "status": "pass" if cycle_state.get("rerun_trigger") == "blocked_interface_stack_control_plane_continuity_state_changed" else "reject",
            "metric": "rerun trigger matches the control-plane continuity trigger contract",
            "value": 1.0 if cycle_state.get("rerun_trigger") == "blocked_interface_stack_control_plane_continuity_state_changed" else 0.0,
        ...
        },
        {
            ...
            "note": "The control-plane operational cycle expects three steps: control-plane audit, control-plane manifest, control-plane continuity settle.",
        },
        {
            "row_id": "apply_chain_signature_present",
            "status": "pass" if apply_chain_signature else "reject",
            "metric": "control-plane rerun/apply chain signature is present",
            "value": 1.0 if apply_chain_signature else 0.0,
            "note": "The chain signature lets downstream readers detect policy drift.",
        },
        {
        ...
        {
            "row_id": "control_plane_complete",
            "status": "pass" if bool(cycle_state.get("control_plane_complete", False)) else "reject",
            "metric": "control-plane pack remains complete",
            "value": 1.0 if bool(cycle_state.get("control_plane_complete", False)) else 0.0,
            "note": "The operational cycle assumes the full blocked control-plane pack is still complete.",
        },
        {
            "row_id": "terminal_control_plane_status_locked_no_change_hold",
            "status": "pass" if terminal_control_plane_status == TERMINAL_CONTROL_PLANE_STATUS else "watch",
            "metric": "terminal control-plane status remains locked on no-change hold",
            "value": 1.0 if terminal_control_plane_status == TERMINAL_CONTROL_PLANE_STATUS else 0.0,
            "note": f"Current terminal_control_plane_status = {terminal_control_plane_status}.",
        },
        {
            "row_id": "mass_origin_branch_blocked",
            "status": "blocked" if bool(cycle_state.get("keep_mass_origin_branch_blocked", True)) else "pass",
            "metric": "mass-origin branch remains blocked in the control-plane operational cycle",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    if has_reject:
        ...
        next_action = "repair_control_plane_trigger_or_apply_chain_contract"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(cycle_state.get("keep_mass_origin_branch_blocked", True))` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_control_plane_operational_cycle"
    else:
        overall_status = "pass"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane operational cycle",
        },
        "inputs": {
        ...
        "intent": "Freeze one current-state operational-cycle decision for the mass-origin blocked interface-stack control-plane pack without executing the rerun chain.",
        "summary": {
            ...
            "terminal_control_plane_status": str(cycle_state.get("terminal_control_plane_status", "")),
            "control_plane_signature_sha256": str(cycle_state.get("control_plane_signature_sha256", "")),
            ...
            "control_plane_complete": bool(cycle_state.get("control_plane_complete", False)),
            "next_required_artifacts": cycle_state.get("next_required_artifacts", []),
        },
        "checks": rows,
        "decision": {
