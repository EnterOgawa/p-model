"""
目的: 黒体 holdout 監査の photon flux per entropy flux 指標図を再生成する。
...
def main() -> None:
    ...
    photon_density_coeff = (16.0 * math.pi * zeta_3 / (speed_of_light**3)) * ((boltzmann_k / planck_h) ** 3)
    photon_flux_coeff = 0.25 * speed_of_light * photon_density_coeff
    entropy_density_coeff = (4.0 / 3.0) * radiation_constant_a
    entropy_flux_coeff = 0.25 * speed_of_light * entropy_density_coeff
    ...
    ratio_photon_flux_per_entropy_flux = photon_flux / np.maximum(1e-300, entropy_flux)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_photon_flux_per_entropy_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_photon_flux_per_entropy_flux, train_idx)
        ratio_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_photon_flux_per_entropy_flux,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_photon_flux_per_entropy_flux, train_idx, exponent=0.0)
    ...
    out_csv = out_dir / "thermo_blackbody_photon_flux_per_entropy_flux_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: Phi_n/Phi_s = const")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_flux_per_entropy_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_flux_per_entropy_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for Phi_n/Phi_s based on blackbody radiation identities.",
