"""
目的: 黒体 holdout 監査の entropy energy ratio 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_entropy_energy = entropy_density / np.maximum(1e-300, energy_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_entropy_energy)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_entropy_energy, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_entropy_energy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_entropy_energy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_entropy_energy, train_idx, exponent=-1.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k ** -1.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_entropy_energy,
        ...
        models["fixed_exponent_minus1_fit"] = {
            "params": {"coeff": float(coeff_fixed), "exponent": -1.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_energy_ratio_holdout_splits.csv"
    ...
    for split_result in split_results:
        ...
        model_fixed = models.get("fixed_exponent_minus1_fit") if isinstance(models, dict) else None
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent -1 (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: s/u = (4/3) T^-1")
    ax.set_title("Blackbody ratio holdout: s/u = (4/3) T^-1")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_energy_ratio_holdout_splits.png"
