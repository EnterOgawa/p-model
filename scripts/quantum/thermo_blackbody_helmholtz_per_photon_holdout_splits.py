"""
目的: 黒体 holdout 監査の helmholtz per photon 指標図を再生成する。
...
def main() -> None:
    ...
    energy_density = radiation_constant_a * (temperature_grid_k**4)
    entropy_density = (4.0 / 3.0) * radiation_constant_a * (temperature_grid_k**3)
    helmholtz_density = energy_density - temperature_grid_k * entropy_density  # f = u - Ts = -(a/3)T^4
    photon_density = photon_density_coeff * (temperature_grid_k**3)

    ratio_helmholtz_per_photon = helmholtz_density / np.maximum(1e-300, photon_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_helmholtz_per_photon))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_helmholtz_per_photon, train_idx)
        ratio_pred = -np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_helmholtz_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_helmholtz_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_helmholtz_per_photon, train_idx, exponent=1.0)
        ratio_pred_fixed = -np.abs(coeff_fixed_abs * (temperature_grid_k**1.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_helmholtz_per_photon,
        ...
        models["fixed_exponent_1_abs_fit"] = {
            "params": {"abs_coeff": float(coeff_fixed_abs), "exponent": 1.0, "sign": -1.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_per_photon_holdout_splits.csv"
    ...
    for split_result in split_results:
        ...
        model_fixed = models.get("fixed_exponent_1_abs_fit") if isinstance(models, dict) else None
    ...
    ax.bar(
        bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 1 abs fit (1p)"
    )
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: f/n ∝ T")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_per_photon_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_per_photon_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "f/n is negative for photon gas; model fit is applied to abs(f/n) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for f/n (Helmholtz free-energy density per photon number density) based on blackbody identities.",
