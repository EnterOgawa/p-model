"""
目的: 黒体 holdout 監査の pressure flux ratio 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_pressure_flux = pressure_density / np.maximum(1e-300, energy_flux)
    ratio_sigma = np.maximum(1e-12, 1e-6 * np.abs(ratio_pressure_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_pressure_flux, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_pressure_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_pressure_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_pressure_flux, train_idx, exponent=0.0)
        ratio_pred_fixed = np.abs(coeff_fixed_abs * (temperature_grid_k**0.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_pressure_flux,
    ...
    out_csv = out_dir / "thermo_blackbody_pressure_flux_ratio_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: P/M = 4/(3c)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_pressure_flux_ratio_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_pressure_flux_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "P/M is positive for photon gas; model fit is applied to abs(P/M) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for P/M = 4/(3c) based on blackbody pressure and energy-flux identities.",
