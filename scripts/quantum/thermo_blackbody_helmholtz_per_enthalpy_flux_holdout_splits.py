"""
目的: 黒体 holdout 監査の helmholtz per enthalpy flux 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_helmholtz_per_enthalpy_flux = helmholtz_density / np.maximum(1e-300, enthalpy_flux)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_helmholtz_per_enthalpy_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_helmholtz_per_enthalpy_flux, train_idx)
        ratio_pred = -np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_helmholtz_per_enthalpy_flux,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(
            temperature_grid_k,
            ratio_helmholtz_per_enthalpy_flux,
            train_idx,
            exponent=0.0,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_per_enthalpy_flux_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: f/Φ_h = const")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_per_enthalpy_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_per_enthalpy_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "f/Φ_h is negative for photon gas; model fit is applied to abs(f/Φ_h) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for f/Φ_h (Helmholtz density per enthalpy flux) based on blackbody identities.",
