...
# -*- coding: utf-8 -*-
"""
mass_origin_geometric_binding_selection_second_retry.py

Step 8.7.55.2.409:
Reinject the reflective-cavity second retry result into the post-linearized
binding selection gate and decide whether the geometric route can now be
selected as a unique reopening channel.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_geometric_binding_selection_second_retry_rows.csv
"""
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_geometric_binding_selection_second_retry_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_geometric_binding_selection_second_retry_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.409"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retry geometric binding-route selection after reflective-cavity second retry.")
...
def _build_rows(
    *,
    ...
    return [
        {
            "row_id": "geometric_binding_selection_second_retry_complete",
            "status": "pass",
            "metric": "geometric binding selection second retry complete",
            "value": 1.0,
            "note": "This step reinjects the reflective-cavity second retry into the post-linearized binding selection gate.",
        },
        {
            "row_id": "selected_binding_route_unique",
            "status": "pass" if selected_binding_route_or_none is not None else "reject",
            "metric": "selected geometric binding route unique after second retry",
        ...
        },
        {
            ...
            "metric": "discrete-spectrum reopen ready after geometric second retry",
        ...
        },
        {
            ...
            "metric": "remaining binding blocker count after geometric second retry",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    for path in (ROUTE_CONTRACT_JSON, SECOND_RETRY_JSON):
        _require_path(path)
    ...
    geometric_boundary_promotion_rule_available = bool(second_retry_summary.get("geometric_boundary_promotion_rule_available", False))
    ...
    remaining_binding_blockers = [] if geometric_boundary_promotion_rule_available else ["shell_quantization_to_domain_relation"]
    selection_second_retry_nonclosure_reason_or_none = None if geometric_boundary_promotion_rule_available else "shell_quantization_reflective_cavity_rule_still_unavailable"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "geometric binding selection second retry",
        },
        "inputs": {
        ...
        "intent": "Decide whether the geometric reflective-boundary route can now be selected as the unique post-linearized reopening channel after the second retry.",
        "formulas": {
            ...
            "current_absence": "the reflective cavity rule remains unavailable because the shell-to-domain relation is still not public canonical",
        },
        "rows": rows,
        "summary": {
            ...
            "remaining_binding_blockers": remaining_binding_blockers,
            "selection_second_retry_nonclosure_reason_or_none": selection_second_retry_nonclosure_reason_or_none,
        },
        "decision": {
            "overall_status": "geometric_binding_selection_second_retry_frozen",
