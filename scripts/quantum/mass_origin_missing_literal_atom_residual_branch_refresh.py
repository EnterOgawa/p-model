...
# -*- coding: utf-8 -*-
"""
mass_origin_missing_literal_atom_residual_branch_refresh.py

Step 8.7.55.2.138:
Reinject the residual missing-literal-atom route results into the blocked
...
PRIOR_REFRESH_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_source_literal_residual_branch_refresh_metrics.json"
SPLIT_CONTRACT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_literal_atom_residual_split_contract_metrics.json"
SHELL_ANCHOR_RETRY_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_shell_anchor_missing_literal_atom_closure_retry_metrics.json"
EXPLICIT_MAPPING_RETRY_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_explicit_mapping_missing_literal_atom_closure_retry_metrics.json"
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_literal_atom_residual_branch_refresh_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_literal_atom_residual_branch_refresh_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.138"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh blocked second-route handoff eligibility after missing-literal-atom residual retries.")
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    target_source_kind = "shell_anchor_missing_literal_atom_bridge" if target_available else None
    ...
    remaining_blockers = _ordered_unique([
        str(shell_anchor_retry_summary.get("shell_anchor_missing_literal_atom_nonclosure_reason_or_none") or ""),
        str(explicit_mapping_retry_summary.get("explicit_mapping_missing_literal_atom_nonclosure_reason_or_none") or ""),
    ])
    rows: List[Dict[str, Any]] = [
        {"row_id": "missing_literal_atom_residual_branch_refresh_complete", "status": "pass", "metric": "missing-literal-atom residual branch refresh complete", "value": 1.0, "note": "This refresh reinjects literal-atom residual retries into the blocked second-route stack and refreezes .83-.84 handoff eligibility."},
        {"row_id": "hand_off_to_8_7_55_2_83", "status": "pass" if handoff else "reject", "metric": "handoff to 8.7.55.2.83-.84 allowed after missing-literal-atom residual refresh", "value": 1.0 if handoff else 0.0, "note": "Handoff to the discrete-spectrum pilot is now allowed." if handoff else "Handoff remains blocked because missing-literal-atom residual reinjection did not reopen the branch."},
        {"row_id": "missing_literal_atom_residual_branch_refresh_source_level_blocker_count", "status": "inventory", "metric": "remaining source-level blocker count after missing-literal-atom residual refresh", "value": float(len(remaining_blockers)), "note": f"Remaining source-level blockers are {remaining_blockers}."},
    ]
    return {
        "generated_utc": _utc_now_iso(),
        "phase": {"phase": 8, "step": step_tag, "name": "missing-literal-atom residual branch refresh / handoff"},
        "inputs": {
            "mass_origin_missing_source_literal_residual_branch_refresh_json": _relative_str(PRIOR_REFRESH_JSON),
            "mass_origin_missing_literal_atom_residual_split_contract_json": _relative_str(SPLIT_CONTRACT_JSON),
            "mass_origin_shell_anchor_missing_literal_atom_closure_retry_json": _relative_str(SHELL_ANCHOR_RETRY_JSON),
            "mass_origin_explicit_mapping_missing_literal_atom_closure_retry_json": _relative_str(EXPLICIT_MAPPING_RETRY_JSON),
        },
        "rows": rows,
        "summary": {
        ...
        "decision": {
            "overall_status": "missing_literal_atom_residual_branch_refresh_reopen_ready" if handoff else "missing_literal_atom_residual_branch_refresh_still_blocked",
