...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_pack_manifest.py

Step 8.7.55.2.36:
Freeze a single public manifest for the current blocked interface-stack
control-plane pack of the mass-origin branch, so downstream readers can
inspect the complete top-level control-plane pack without reopening each
JSON artifact manually.
...
Assumptions:
  - The control-plane pack is complete only if every current top-level
    control-plane artifact and the control-plane pack audit exist together.
...
CONTROL_PLANE_MANIFEST_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_manifest.json"
CONTROL_PLANE_CONTINUITY_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_continuity_metrics.json"
CONTROL_PLANE_RERUN_POLICY_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_rerun_policy.json"
CONTROL_PLANE_OPERATIONAL_CYCLE_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_operational_cycle.json"
CONTROL_PLANE_PACK_AUDIT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_metrics.json"
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_manifest.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_manifest_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.36"

ARTIFACT_SPECS = [
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_manifest",
        "role": "control_plane_pack_input",
        "path": CONTROL_PLANE_MANIFEST_JSON,
        "note": "Frozen manifest for the current control-plane layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_continuity",
        "role": "control_plane_pack_input",
        "path": CONTROL_PLANE_CONTINUITY_JSON,
        "note": "Continuity audit for the current control-plane layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_rerun_policy",
        "role": "control_plane_pack_input",
        "path": CONTROL_PLANE_RERUN_POLICY_JSON,
        "note": "Rerun/apply policy for the current control-plane layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_operational_cycle",
        "role": "control_plane_pack_input",
        "path": CONTROL_PLANE_OPERATIONAL_CYCLE_JSON,
        "note": "Operational-cycle decision for the current control-plane layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_audit",
        "role": "control_plane_pack_audit",
        "path": CONTROL_PLANE_PACK_AUDIT_JSON,
        "note": "Top-level consistency audit across the current control-plane pack.",
    },
]
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze a single manifest for the current mass-origin interface-stack control-plane pack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    terminal_row = next(
        row for row in rows if row["row_id"] == "mass_origin_blocked_interface_stack_control_plane_pack_audit"
    )
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane pack manifest",
        },
        "inputs": {
            "public_quantum_dir": _rel(PUBLIC_QUANTUM_DIR),
            "artifact_count": len(rows),
        },
        "intent": "Freeze the current blocked interface-stack control-plane pack of the mass-origin branch as a single public canonical manifest.",
        "rows": rows,
        "summary": {
            "artifact_count": len(rows),
            ...
            "terminal_control_plane_pack_status": str(terminal_row.get("artifact_status", "")),
            "control_plane_signature_sha256": str(terminal_summary.get("control_plane_signature_sha256", "")),
        ...
        "decision": {
            "overall_status": "blocked_interface_stack_control_plane_pack_manifest_frozen",
            "control_plane_complete": bool(terminal_decision.get("control_plane_complete", True)),
            ...
            "next_required_artifacts": next_required_artifacts,
            "next_action": "wait_for_new_public_artifact_then_refresh_control_plane_pack_manifest",
        },
        "evidence": {
            "terminal_control_plane_pack_audit": {
