"""
目的: 黒体 holdout 監査の entropy per photon 指標図を再生成する。
...
def main() -> None:
    ...
    entropy_per_photon = entropy_density / photon_density

    ratio_sigma = np.maximum(1e-12, 1e-6 * entropy_per_photon)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, entropy_per_photon, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=entropy_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=entropy_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, entropy_per_photon, train_idx, exponent=0.0)
        ratio_pred_fixed = coeff_fixed * np.ones_like(temperature_grid_k)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=entropy_per_photon,
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_per_photon_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(
        ...
        title="Blackbody entropy per photon holdout audit" if _is_en_figure() else "黒体エントロピー/光子の温度帯分割監査",
    )
    ...
    out_png = out_dir / "thermo_blackbody_entropy_per_photon_holdout_splits.png"
    ...
    out_metrics = out_dir / "thermo_blackbody_entropy_per_photon_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody entropy per photon s/n = (2π^4/(45 ζ(3))) k_B.",
