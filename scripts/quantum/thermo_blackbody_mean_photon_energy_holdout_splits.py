"""
目的: 黒体 holdout 監査の mean photon energy 指標図を再生成する。
...
def main() -> None:
    ...
    mean_energy_coeff = (math.pi**4) / (30.0 * zeta3) * boltzmann_k

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    mean_energy_j = mean_energy_coeff * temperature_grid_k

    mean_energy_sigma = np.maximum(1e-12, 1e-6 * mean_energy_j)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, mean_energy_j, train_idx)
        energy_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=mean_energy_j, y_pred=energy_pred, sigma=mean_energy_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=mean_energy_j, y_pred=energy_pred, sigma=mean_energy_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, mean_energy_j, train_idx, exponent=1.0)
        energy_pred_fixed = coeff_fixed * temperature_grid_k
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=mean_energy_j,
            y_pred=energy_pred_fixed,
            sigma=mean_energy_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_mean_photon_energy_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(
        ...
        title="黒体平均光子エネルギーの温度帯分割監査: 平均光子エネルギー ∝ T",
    )
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_mean_photon_energy_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_mean_photon_energy_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {"kB_J_per_K": boltzmann_k, "zeta3": zeta3},
                "assumptions": {
                ...
                "notes": [
                    "Analytic holdout for blackbody mean photon energy ⟨E⟩=(π^4/(30 ζ(3))) k_B T.",
