...
# -*- coding: utf-8 -*-
"""
mass_origin_missing_source_literal_residual_branch_refresh.py

Step 8.7.55.2.132:
Reinject the residual missing-literal-fragment route results into the blocked
...
Outputs:
  ...
  - output/public/quantum/mass_origin_missing_source_literal_residual_branch_refresh_rows.csv
"""
...
PRIOR_REFRESH_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_slot_source_residual_branch_refresh_metrics.json"
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_source_literal_residual_branch_refresh_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_source_literal_residual_branch_refresh_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.132"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh blocked second-route handoff eligibility after missing-literal-fragment residual retries.",
    )
    parser.add_argument(
        "--step-tag",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    remaining_source_level_blockers = _ordered_unique(
        [
            str(shell_anchor_retry_summary.get("shell_anchor_missing_literal_fragment_nonclosure_reason_or_none") or ""),
            str(explicit_mapping_retry_summary.get("explicit_mapping_missing_literal_fragment_nonclosure_reason_or_none") or ""),
        ]
    )
    source_routes_still_admissible = bool(
        split_contract_summary.get("shell_anchor_missing_literal_route_still_admissible", False)
        and split_contract_summary.get("explicit_mapping_missing_literal_route_still_admissible", False)
    )

    rows = [
        {
            "row_id": "missing_source_literal_residual_branch_refresh_complete",
            "status": "pass",
            "metric": "missing-source-literal residual branch refresh complete",
            "value": 1.0,
            "note": "This refresh reinjects literal-fragment residual retries into the blocked second-route stack and refreezes .83-.84 handoff eligibility.",
        },
        {
            "row_id": "missing_source_literal_residual_branch_source_routes_admissible",
            "status": "pass" if source_routes_still_admissible else "reject",
            "metric": "missing-source-literal residual routes remain admissible",
            "value": 1.0 if source_routes_still_admissible else 0.0,
            "note": (
                "Both shell-anchor and explicit-mapping literal-fragment routes remain admissible."
                if source_routes_still_admissible
                else "At least one literal-fragment route is no longer admissible, so the refresh cannot support handoff."
            ),
        },
        {
            "row_id": "missing_source_literal_residual_branch_same_sector_tiebreak_target_value",
            "status": "pass" if same_sector_tiebreak_target_value_available else "watch",
            "metric": "same-sector tie-break target value available after missing-source-literal residual refresh",
            "value": 1.0 if same_sector_tiebreak_target_value_available else 0.0,
            "note": (
                f"Target source kind is {target_source_kind_or_none}."
                if same_sector_tiebreak_target_value_available
                else f"Literal-fragment residual refresh remains blocked by {remaining_source_level_blockers}."
            ),
        },
        {
            "row_id": "missing_source_literal_residual_branch_single_public_vpp_shape",
            "status": "pass" if single_public_vpp_shape_available else "reject",
            "metric": "single public V(|P|) shape available after missing-source-literal residual refresh",
        ...
        {
            "row_id": "missing_source_literal_residual_branch_positive_same_sector_public_artifact",
            "status": "pass" if positive_particle_sector_chi_p_to_vpp_public_artifact_available else "reject",
            "metric": "positive particle-sector chi_P -> V''(|P|_*) public artifact available after missing-source-literal residual refresh",
            ...
            "note": (
                ...
                else "Promotion remains absent because explicit mapping missing-literal-fragment closure and single-shape closure did not reopen together."
            ),
        },
        {
            "row_id": "solver_ready_row_promoted_to_pass",
            "status": "pass" if solver_ready_row_promoted_to_pass else "reject",
            "metric": "solver-ready row promoted to pass after missing-source-literal residual refresh",
            "value": 1.0 if solver_ready_row_promoted_to_pass else 0.0,
            "note": (
                "All reopen prerequisites now close simultaneously after missing-source-literal residual reinjection."
        ...
        },
        {
            ...
            "metric": "mass-origin branch reopen ready after missing-source-literal residual refresh",
        ...
        },
        {
            ...
            "metric": "handoff to 8.7.55.2.83-.84 allowed after missing-source-literal residual refresh",
            ...
            "note": (
                ...
                else "Handoff remains blocked because missing-source-literal residual reinjection did not reopen the branch."
            ),
        },
        {
            "row_id": "missing_source_literal_residual_branch_refresh_source_level_blocker_count",
            "status": "inventory",
            "metric": "remaining source-level blocker count after missing-source-literal residual refresh",
            ...
            "note": (
                ...
                else "No source-level blockers remain after missing-source-literal residual refresh."
            ),
        },
        {
            "row_id": "missing_source_literal_residual_branch_refresh_remaining_artifact_count",
            "status": "inventory",
            "metric": "remaining artifact-level missing count after missing-source-literal residual refresh",
            ...
            "note": (
                ...
                else "No artifact-level blockers remain after missing-source-literal residual refresh."
            ),
        },
    ]

    overall_status = (
        "missing_source_literal_residual_branch_refresh_reopen_ready"
        if hand_off_to_8_7_55_2_83
        else "missing_source_literal_residual_branch_refresh_still_blocked"
    )
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "missing-source-literal residual branch refresh / handoff",
        },
        "inputs": {
            "mass_origin_missing_slot_source_residual_branch_refresh_json": _relative_str(PRIOR_REFRESH_JSON),
        ...
        "intent": "Refresh the blocked second-route stack after literal-fragment residual retries and refreeze whether the branch can hand off to .83-.84.",
        "formulas": {
            "target_value_rule": "same_sector_tiebreak_target_value_available iff the shell-anchor missing-literal-fragment retry now closes an admissible literal-fragment route",
            "single_shape_rule": "single_public_vpp_shape_available remains tied to the prior refreshed candidate selection unless the new target-driven literal-fragment route reopens it",
