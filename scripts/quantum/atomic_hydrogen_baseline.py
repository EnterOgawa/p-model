...
def main() -> None:
    ...
    title_font = get_wavep_font_size("title") * (0.90 if is_en else 1.0)
    axis_font = get_wavep_font_size("axis") * (0.92 if is_en else 1.0)
    tick_font = get_wavep_font_size("tick")
    line_label_font = get_wavep_font_size("note") * (0.90 if is_en else 1.0)
    ...
    ax.set_title("Atomic baseline (Hydrogen, NIST ASD)", fontsize=title_font)
    ...
    for i, r in enumerate(lines_out):
        ...
        label = multiplet_label.get(raw_label, raw_label.replace("_", " "))
    ...
    out_pdf = localize_figure_output_path(out_dir / "atomic_hydrogen_baseline.pdf", root=root)
    out_png = localize_figure_output_path(out_dir / "atomic_hydrogen_baseline.png", root=root)
    ...
    out_json = localize_figure_output_path(out_dir / "atomic_hydrogen_baseline_metrics.json", root=root)
