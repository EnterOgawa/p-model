...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_evidence_pack_rerun_policy.py

Step 8.7.55.2.23:
Freeze the rerun/apply policy for the mass-origin blocked evidence pack, so
downstream operators know which public canonical scripts must be rerun, and in
what order, when the blocked-pack continuity state changes.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_evidence_pack_rerun_policy_rows.csv

Assumptions:
  - The blocked evidence-pack continuity audit is the sole trigger source for
    deciding whether the blocked-pack reader chain should be rerun.
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_evidence_pack_rerun_policy.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_evidence_pack_rerun_policy_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.23"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the rerun/apply policy for the mass-origin blocked evidence pack.",
    )
    parser.add_argument(
        "--step-tag",
    ...
    chain_specs = [
        {
            ...
            "run_when": "continuity_state_changed",
        ...
        },
        {
            ...
            "settle_required": False,
            "note": "Recompute named missing-artifact gate after latent inventory changes.",
        },
        {
            ...
            "settle_required": False,
            "note": "Rebuild blocked-state decision from the refreshed blocker artifacts.",
        },
        {
        ...
        },
        {
            ...
            "run_when": "after_manifest_refresh",
            "settle_required": True,
            "note": "Rerun until the continuity artifact returns no_change_hold against the refreshed manifest pack.",
        },
    ]
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    # 条件分岐: 前段条件が不成立で、`continuity_state.get(\"continuity_decision\") == \"no_change_hold\"` を追加評価する。
        ...
        overall_status = "watch"
        next_action = "wait_for_new_public_artifact_then_rerun_policy_chain"
    else:
        decision_text = "policy_locked_watch_state"
        overall_status = "watch"
        next_action = str(continuity_state.get("watchpack_next_action") or "keep_policy_active_until_continuity_changes")
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked evidence-pack rerun policy",
        },
        "inputs": {
            "blocked_evidence_pack_continuity_metrics_json": _rel(CONTINUITY_JSON),
            "blocked_evidence_pack_manifest_json": _rel(MANIFEST_JSON),
        },
        "intent": "Freeze the rerun/apply chain that must be used when the mass-origin blocked evidence-pack continuity state changes.",
        "summary": {
        ...
        "decision": {
            ...
            "rerun_required_now": rerun_required_now,
            "rerun_trigger": "blocked_evidence_pack_continuity_state_changed",
