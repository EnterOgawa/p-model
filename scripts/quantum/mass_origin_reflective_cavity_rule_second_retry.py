...
# -*- coding: utf-8 -*-
"""
mass_origin_reflective_cavity_rule_second_retry.py

Step 8.7.55.2.408:
Retry closure of the shell-quantization reflective cavity rule after the
boundary-radius proxy source inventory and wording audit.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_reflective_cavity_rule_second_retry_rows.csv
"""
...
BOUNDARY_PROXY_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_boundary_radius_proxy_wording_audit_metrics.json"
FIRST_RETRY_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_reflective_cavity_rule_retry_metrics.json"
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_reflective_cavity_rule_second_retry_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_reflective_cavity_rule_second_retry_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.408"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Retry closure of the shell-quantization reflective cavity rule a second time.")
...
def _build_rows(
    *,
    ...
    cavity_rule_second_retry_nonclosure_reason_or_none: str | None,
) -> List[Dict[str, Any]]:
    return [
        {
            "row_id": "reflective_cavity_rule_second_retry_complete",
            "status": "pass",
            "metric": "reflective cavity rule second retry complete",
            "value": 1.0,
            "note": "This step retries closure of the shell-quantization reflective cavity rule after the boundary-radius proxy residual branch.",
        },
        {
            "row_id": "boundary_radius_or_domain_available",
            "status": "pass" if boundary_radius_or_domain_available else "reject",
            "metric": "boundary radius or domain available for reflective cavity rule second retry",
        ...
        },
        {
            ...
            "metric": "geometric boundary promotion rule available after second retry",
        ...
        },
        {
            ...
            "metric": "discrete shell cavity ready after second retry",
        ...
        {
            "row_id": "cavity_rule_second_retry_nonclosure_reason",
            "status": "watch" if cavity_rule_second_retry_nonclosure_reason_or_none else "pass",
            "metric": "reflective cavity rule second-retry non-closure reason",
            "value": 0.0 if cavity_rule_second_retry_nonclosure_reason_or_none else 1.0,
            "note": (
                f"Current non-closure reason: {cavity_rule_second_retry_nonclosure_reason_or_none}."
                if cavity_rule_second_retry_nonclosure_reason_or_none
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    for path in (ROUTE_CONTRACT_JSON, SOURCE_INVENTORY_JSON, BOUNDARY_PROXY_JSON, FIRST_RETRY_JSON):
        _require_path(path)
    ...
    first_retry = _read_json(FIRST_RETRY_JSON)
    ...
    first_retry_summary = first_retry.get("summary", {})
    ...
    cavity_rule_second_retry_nonclosure_reason_or_none = None

    if not geometric_boundary_promotion_rule_available:
        cavity_rule_second_retry_nonclosure_reason_or_none = "shell_quantization_to_domain_relation_absent"
    ...
    rows = _build_rows(
        ...
        cavity_rule_second_retry_nonclosure_reason_or_none=cavity_rule_second_retry_nonclosure_reason_or_none,
    )
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "reflective cavity rule second retry",
        },
        "inputs": {
            ...
            "mass_origin_boundary_radius_proxy_wording_audit_json": _relative_str(BOUNDARY_PROXY_JSON),
            "mass_origin_reflective_cavity_rule_retry_json": _relative_str(FIRST_RETRY_JSON),
        },
        "intent": "Retry closure of the shell-quantization reflective cavity rule after the boundary-radius proxy residual branch.",
        "formulas": {
            "closure_rule": "the reflective cavity route closes only if shell quantization is linked to a cavity domain without a new scale and the proxy can be injected into the mexican-hat pilot as a reflective wall",
            "current_absence": "the public shell-family coefficients still lack the shell-to-domain relation needed to define a cavity radius or domain proxy",
        },
        "rows": rows,
        "summary": {
        ...
        "decision": {
            "overall_status": (
                "reflective_cavity_rule_second_retry_closed"
                if geometric_boundary_promotion_rule_available
                else "reflective_cavity_rule_second_retry_still_blocked"
            ),
            ...
            "next_required_artifacts": (
                ["geometric_binding_selection_second_retry", "discrete_spectrum_third_reopen_refresh"]
                if geometric_boundary_promotion_rule_available
                else ["shell_quantization_to_domain_relation", "boundary_radius_or_domain_proxy", "shell_quantization_reflective_cavity_rule"]
            ),
        },
        "evidence": {
            ...
            "boundary_radius_proxy_wording_audit_summary": boundary_proxy_summary,
            "reflective_cavity_rule_retry_summary": first_retry_summary,
        },
    }
