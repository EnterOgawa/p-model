...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_continuity_audit.py

Step 8.7.55.2.32:
Freeze a downstream continuity audit for the mass-origin blocked
interface-stack control-plane manifest, so later readers can detect whether
the full control-plane pack changed as a single public canonical unit.

Inputs:
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_manifest.json
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_manifest_rows.csv

Outputs:
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_continuity_metrics.json
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_continuity_rows.csv

Assumptions:
  - The control-plane manifest is the single public canonical summary of the
    current top-level blocked interface-stack control-plane layer.
...
MANIFEST_ROWS_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_manifest_rows.csv"
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_continuity_metrics.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_continuity_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.32"
TERMINAL_CONTROL_PLANE_STATUS = "watch:blocked_interface_stack_control_plane_locked_no_change_hold"
...
def _derive_continuity_watchpack(
    *,
    ...
    # 条件分岐: 前段条件が不成立で、`continuity_state_changed` を追加評価する。
        ...
        update_event_type = "blocked_interface_stack_control_plane_state_changed"
    ...
    if continuity_state_changed:
        next_action = "rerun_control_plane_readers_now"
    # 条件分岐: 前段条件が不成立で、`logic_state.get(\"keep_mass_origin_branch_blocked\", True)` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_control_plane_continuity"
    else:
    ...
    return {
        ...
        "note": (
            "Event counter increments only when the logical control-plane state changes. "
...
def _build_rows(logic_state: Dict[str, Any], watchpack: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        {
            "row_id": "blocked_interface_stack_control_plane_manifest_signature_present",
            "status": "pass" if str(logic_state.get("manifest_signature_sha256") or "") else "reject",
            "metric": "blocked interface-stack control-plane manifest signature is present",
            "value": 1.0 if str(logic_state.get("manifest_signature_sha256") or "") else 0.0,
            "note": "Continuity audit requires a stable top-level control-plane manifest signature.",
        },
        {
            "row_id": "blocked_interface_stack_control_plane_rows_sync",
            "status": "pass" if bool(logic_state.get("rows_csv_matches_manifest", False)) else "reject",
            "metric": "control-plane rows CSV count matches manifest summary count",
        ...
        },
        {
            ...
            "metric": "blocked-state detail propagated into control-plane continuity",
        ...
        {
            "row_id": "blocked_interface_stack_control_plane_signature_present",
            "status": "pass" if str(logic_state.get("control_plane_signature_sha256") or "") else "reject",
            "metric": "control-plane signature is present",
            "value": 1.0 if str(logic_state.get("control_plane_signature_sha256") or "") else 0.0,
            "note": "The continuity layer requires the upstream control-plane audit signature to remain available.",
        },
        {
            "row_id": "blocked_interface_stack_control_plane_complete",
            "status": "pass" if bool(logic_state.get("control_plane_complete", False)) else "reject",
            "metric": "blocked interface-stack control-plane remains complete",
            "value": 1.0 if bool(logic_state.get("control_plane_complete", False)) else 0.0,
            "note": "Continuity audit assumes the top-level control-plane pack remains complete before downstream readers rely on it.",
        },
        {
            "row_id": "mass_origin_branch_blocked_in_control_plane_manifest",
            "status": "blocked" if bool(logic_state.get("keep_mass_origin_branch_blocked", True)) else "pass",
            "metric": "control-plane manifest keeps the mass-origin branch blocked",
        ...
        {
            "row_id": "control_plane_manifest_continuity_no_change_hold",
            "status": "pass" if bool(logic_state.get("continuity_no_change_hold", False)) else "watch",
            "metric": "control-plane manifest still records continuity no-change hold",
            "value": 1.0 if bool(logic_state.get("continuity_no_change_hold", False)) else 0.0,
            "note": "The top-level control-plane manifest should retain the settled no-change-hold marker from the upstream continuity layer.",
        },
        {
            "row_id": "terminal_control_plane_status_locked_no_change_hold",
            "status": "pass" if str(logic_state.get("terminal_control_plane_status", "")) == TERMINAL_CONTROL_PLANE_STATUS else "watch",
            "metric": "terminal control-plane status remains locked on no-change hold",
            "value": 1.0 if str(logic_state.get("terminal_control_plane_status", "")) == TERMINAL_CONTROL_PLANE_STATUS else 0.0,
            "note": f"Current terminal_control_plane_status = {logic_state.get('terminal_control_plane_status')}.",
        },
        {
            "row_id": "blocked_interface_stack_control_plane_continuity_state_settled",
            "status": "pass" if state_settled else "watch",
            "metric": "blocked interface-stack control-plane logical state is settled",
            "value": 1.0 if state_settled else 0.0,
            "note": (
                "Control-plane continuity state is unchanged from the previous snapshot."
                if state_settled
                else "Control-plane continuity state changed and downstream readers should be reevaluated."
            ),
        },
        {
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    current_input_signatures = {
        "blocked_interface_stack_control_plane_manifest_json": _file_signature(MANIFEST_JSON),
        "blocked_interface_stack_control_plane_manifest_rows_csv": _file_signature(MANIFEST_ROWS_CSV),
    }
    ...
    if has_reject:
        ...
        decision_text = "blocked_interface_stack_control_plane_sync_broken"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(watchpack.get("continuity_state_changed", False))` を追加評価する。
        ...
        overall_status = "watch"
        decision_text = "blocked_interface_stack_control_plane_state_changed"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane continuity audit",
        },
        "inputs": {
            "blocked_interface_stack_control_plane_manifest_json": _rel(MANIFEST_JSON),
            "blocked_interface_stack_control_plane_manifest_rows_csv": _rel(MANIFEST_ROWS_CSV),
        },
        ...
        "summary": {
            ...
            "terminal_control_plane_status": str(logic_state.get("terminal_control_plane_status", "")),
            "control_plane_signature_sha256": str(logic_state.get("control_plane_signature_sha256", "")),
        ...
        "decision": {
            ...
            "control_plane_complete": bool(logic_state.get("control_plane_complete", False)),
