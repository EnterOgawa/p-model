"""
目的: 黒体 holdout 監査の helmholtz enthalpy ratio 指標図を再生成する。
...
def main() -> None:
    ...
    enthalpy_density = (4.0 * radiation_constant_a / 3.0) * (temperature_grid_k**4)

    ratio_free_energy_enthalpy = free_energy_density / np.maximum(1e-300, enthalpy_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * np.abs(ratio_free_energy_enthalpy))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_free_energy_enthalpy, train_idx)
        ratio_pred_abs = (10**log10_coeff) * (temperature_grid_k**exponent)
        ratio_pred = -np.abs(ratio_pred_abs)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_free_energy_enthalpy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_free_energy_enthalpy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_free_energy_enthalpy, train_idx, exponent=0.0)
        ratio_pred_fixed = -np.abs(coeff_fixed_abs * np.ones_like(temperature_grid_k))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_free_energy_enthalpy,
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_enthalpy_ratio_holdout_splits.csv"
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 0 abs fit (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: f/h = -1/4")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_enthalpy_ratio_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_enthalpy_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "f/h is negative for photon gas; model fit is applied to abs(f/h) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for f/h = -1/4 based on blackbody thermodynamic identities.",
