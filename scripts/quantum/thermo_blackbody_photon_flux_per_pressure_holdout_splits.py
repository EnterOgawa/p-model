...
def main() -> None:
    ...
    ratio_photon_flux_per_pressure = photon_flux / np.maximum(1e-300, pressure_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_photon_flux_per_pressure))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_photon_flux_per_pressure, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_photon_flux_per_pressure,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_photon_flux_per_pressure, train_idx, exponent=-1.0)
    ...
    out_csv = out_dir / "thermo_blackbody_photon_flux_per_pressure_holdout_splits.csv"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "Φ_n/P is positive for photon gas; model fit is applied to abs(Φ_n/P) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for Φ_n/P (photon flux per radiation pressure) based on blackbody identities.",
