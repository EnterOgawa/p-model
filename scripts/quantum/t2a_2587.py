#!/usr/bin/env python3
"""Generate 8.7.56.2587-.2590 background-expansion gate artifacts."""
...
STEP_TAG = "8.7.56.2587-2590"
STEP_NAME = "Trial-2 numeric alpha vector Q-ball form-factor updated-pack background-expansion gate"
...
NEXT_ROUTE = "8.7.56.2591"
FOLLOWUP_ROUTE_NAME = (
    "trial2_numeric_alpha_vector_qball_form_factor_updated_pack_charge_current_gate_"
    "low_order_jeff0_refresh"
)
FOLLOWUP_ROUTE = "8.7.56.2595"
...
def build_formulae() -> dict[str, str]:
    """Return formulas used in the background-expansion gate."""
    return {
        "gate_a": "Gate A = exact Q-ball background expansion derived and available now",
        "gate_b": "Gate B = exact charge-current / Noether-current derivation selected as the next primary remaining closure",
        "gate_c": "Gate C = blind-vector computation primary admissible now",
        "ordered_refresh": "background expansion derivation -> charge-current derivation -> low-order J_eff^0 synthesis -> blind vector refresh",
    }
...
def main() -> None:
    """Execute the updated-pack background-expansion gate."""
    for path in (
        STATUS,
        ROADMAP,
        AI_CONTEXT,
    ...
    gate_a_updated_pack_exact_qball_background_expansion_available = bool(
        prior_summary["updated_pack_exact_qball_background_expansion_available_now"]
        and prior_summary["updated_pack_exact_qball_background_expansion_closes_first_missing_primitive_now"]
    )
    gate_b_updated_pack_exact_charge_current_primary_selected = bool(
        prior_summary["updated_pack_exact_charge_current_primary_refresh_required"]
        and gate_a_updated_pack_exact_qball_background_expansion_available
    )
    ...
    rows = [
        sign_base.row(
            "gate_a_updated_pack_exact_qball_background_expansion_available",
            "pass" if gate_a_updated_pack_exact_qball_background_expansion_available else "reject",
            "Gate A updated-pack exact Q-ball background expansion available",
            sign_base.truth(gate_a_updated_pack_exact_qball_background_expansion_available),
            "The background-expansion primitive is now explicit as a derived formula bundle rather than only a target surface.",
        ),
        sign_base.row(
            ...
            "Once the background primitive is derived, the next honest remaining closure is exact charge-current / Noether-current derivation.",
        ),
        sign_base.row(
        ...
        sign_base.row(
            ...
            "The gate synchronizes that the background primitive is no longer absent.",
        ),
        sign_base.row(
            ...
            "The source theorem remains downstream of charge-current closure and low-order J_eff^0.",
        ),
        sign_base.row(
    ...
    summary = {
        ...
        "gate_a_updated_pack_exact_qball_background_expansion_available": gate_a_updated_pack_exact_qball_background_expansion_available,
        ...
        "selected_primary_pack_update_surface": "updated_pack_exact_charge_current_noether_derivation",
    ...
    route_payload = {
        ...
        "evidence": {
            ...
            "disposition": {
                "background_expansion_available_now": exact_qball_background_expansion_available_now,
