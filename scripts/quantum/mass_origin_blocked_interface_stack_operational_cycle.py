...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_operational_cycle.py

Step 8.7.55.2.29:
Freeze one current-state operational-cycle artifact for the mass-origin
blocked interface stack, so downstream readers can see whether the continuity
artifact and the rerun policy agree on the present no-change-hold state.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_interface_stack_operational_cycle_rows.csv
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_operational_cycle.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_operational_cycle_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.29"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the operational-cycle state for the mass-origin blocked interface stack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _extract_cycle_state(continuity: Dict[str, Any], rerun_policy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "hold_monitor_no_change_hold": bool(rerun_decision.get("hold_monitor_no_change_hold", False)),
        "interface_stack_complete": bool(rerun_decision.get("interface_stack_complete", False)),
        "keep_mass_origin_branch_blocked": bool(rerun_decision.get("keep_mass_origin_branch_blocked", True)),
        "terminal_stack_status": str(
            continuity_summary.get("terminal_stack_status", rerun_summary.get("terminal_stack_status", ""))
        ).strip(),
        "terminal_stack_signature_sha256": str(
            continuity_summary.get(
                "terminal_stack_signature_sha256",
                rerun_summary.get("terminal_stack_signature_sha256", ""),
            )
        ).strip(),
...
def _build_rows(cycle_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    terminal_stack_status = str(cycle_state.get("terminal_stack_status", ""))
    ...
    return [
        ...
        },
        {
            ...
            "metric": "blocked interface-stack continuity state is unchanged",
            "value": 1.0 if not bool(cycle_state.get("continuity_state_changed", False)) else 0.0,
            "note": "The rerun chain should stay idle until the continuity state changes.",
        },
        {
            "row_id": "rerun_required_now_false",
            "status": "pass" if not bool(cycle_state.get("rerun_required_now", False)) else "watch",
            "metric": "rerun policy does not currently require the chain to run",
            "value": 1.0 if not bool(cycle_state.get("rerun_required_now", False)) else 0.0,
            "note": "This row is watch only when the policy already says the chain must run now.",
        },
        {
            "row_id": "rerun_trigger_is_continuity_state_changed",
            "status": "pass" if cycle_state.get("rerun_trigger") == "blocked_interface_stack_continuity_state_changed" else "reject",
            "metric": "rerun trigger matches the interface-stack continuity trigger contract",
            "value": 1.0 if cycle_state.get("rerun_trigger") == "blocked_interface_stack_continuity_state_changed" else 0.0,
            "note": f"Current rerun trigger = {cycle_state.get('rerun_trigger')}.",
        },
        {
            "row_id": "apply_chain_step_count_expected",
            "status": "pass" if int(cycle_state.get("apply_chain_step_count", 0)) == 11 else "reject",
            "metric": "rerun/apply chain keeps the expected eleven-step order",
            "value": float(cycle_state.get("apply_chain_step_count", 0)),
            "note": (
                "The operational cycle expects eleven steps: latent inventory, same-sector gate, "
                "blocked-state reopen, hold monitor settle, evidence-pack manifest, evidence-pack continuity, "
                "evidence-pack rerun policy, evidence-pack operational cycle, interface-stack audit, "
                "interface-stack manifest, interface-stack continuity settle."
            ),
        },
        {
            "row_id": "apply_chain_signature_present",
            "status": "pass" if apply_chain_signature else "reject",
            "metric": "rerun/apply chain signature is present",
        ...
        },
        {
            ...
            "status": "pass" if bool(cycle_state.get("hold_monitor_no_change_hold", False)) else "watch",
            "metric": "hold monitor remains settled at no_change_hold",
            "value": 1.0 if bool(cycle_state.get("hold_monitor_no_change_hold", False)) else 0.0,
            "note": "The operational cycle should not escalate while the hold monitor is already settled.",
        },
        {
            "row_id": "interface_stack_complete",
            "status": "pass" if bool(cycle_state.get("interface_stack_complete", False)) else "reject",
            "metric": "interface stack remains complete",
            "value": 1.0 if bool(cycle_state.get("interface_stack_complete", False)) else 0.0,
            "note": "The operational cycle assumes the full blocked interface stack is still complete.",
        },
        {
            "row_id": "terminal_stack_status_locked_no_change_hold",
            "status": "pass" if terminal_stack_status == "watch:blocked_interface_stack_locked_no_change_hold" else "watch",
            "metric": "terminal stack status remains locked on no-change hold",
            "value": 1.0 if terminal_stack_status == "watch:blocked_interface_stack_locked_no_change_hold" else 0.0,
            "note": f"Current terminal_stack_status = {terminal_stack_status}.",
        },
        {
            "row_id": "mass_origin_branch_blocked",
            "status": "blocked" if bool(cycle_state.get("keep_mass_origin_branch_blocked", True)) else "pass",
            "metric": "mass-origin branch remains blocked in the operational cycle",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    if has_reject:
        ...
        next_action = "repair_interface_stack_trigger_or_apply_chain_contract"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(cycle_state.get(\"keep_mass_origin_branch_blocked\", True))` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_interface_stack_operational_cycle"
    else:
        overall_status = "pass"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack operational cycle",
        },
        "inputs": {
        ...
        "intent": "Freeze one current-state operational-cycle decision for the mass-origin blocked interface stack without executing the rerun chain.",
        "summary": {
            ...
            "terminal_stack_status": str(cycle_state.get("terminal_stack_status", "")),
            "terminal_stack_signature_sha256": str(cycle_state.get("terminal_stack_signature_sha256", "")),
            ...
            "hold_monitor_no_change_hold": bool(cycle_state.get("hold_monitor_no_change_hold", False)),
            "interface_stack_complete": bool(cycle_state.get("interface_stack_complete", False)),
            "next_required_artifacts": cycle_state.get("next_required_artifacts", []),
        },
        "checks": rows,
        "decision": {
