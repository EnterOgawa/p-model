"""
目的: 黒体 holdout 監査の helmholtz free energy density 指標図を再生成する。
...
def main() -> None:
    ...
    helmholtz_coeff_abs = radiation_constant_a / 3.0

    temperature_grid_k = np.logspace(0.0, 4.0, 80)
    free_energy_density = -helmholtz_coeff_abs * (temperature_grid_k**4)
    free_energy_sigma = np.maximum(1e-12, 1e-6 * np.abs(free_energy_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, free_energy_density, train_idx)
        free_pred_abs = (10**log10_coeff) * (temperature_grid_k**exponent)
        free_pred = -np.abs(free_pred_abs)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=free_energy_density, y_pred=free_pred, sigma=free_energy_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=free_energy_density, y_pred=free_pred, sigma=free_energy_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, free_energy_density, train_idx, exponent=4.0)
        free_pred_fixed = -np.abs(coeff_fixed_abs * (temperature_grid_k**4.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=free_energy_density,
            y_pred=free_pred_fixed,
            sigma=free_energy_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_free_energy_density_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody Helmholtz free-energy density holdout: f(T)=u(T)-T s(T)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_free_energy_density_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_free_energy_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "helmholtz_abs_coeff": helmholtz_coeff_abs,
                },
                "assumptions": {
                    "sign_convention": "f(T) is negative for photon gas; model fit is applied to abs(f) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for blackbody Helmholtz free-energy density f(T)=u(T)-T*s(T)=-(a/3)T^4.",
