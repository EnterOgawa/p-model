#!/usr/bin/env python3
"""Generate 8.7.56.2247-.2250 hybrid seventh/eighth farther-continuation artifacts."""
...
PRIOR_AUDIT = (
    base.PUBLIC_OUT
    / "q_8_7_56_2239_2242_harmonic_hybrid_s6_s7_fast_declaration_gate_metrics.json"
)
PRIOR_REGISTRY = (
    base.PUBLIC_OUT
    / "q_8_7_56_2243_2246_harmonic_hybrid_s6_s7_registry_declaration_gate_metrics.json"
)

STEP_TAG = "8.7.56.2247-2250"
STEP_NAME = (
    "Trial-2 numeric alpha vector Q-ball form-factor hybrid seventh/eighth "
    "farther audit"
)
STEM = build_compact_artifact_stem(STEP_TAG, "harmonic_hybrid_s7_s8_fast", prefix="q")
base.STEM = STEM

PRIOR_CLASS = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_promoted_1376256_next"
)
BRANCH_CLASS = "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_s8_fast_gate"
NEXT_ROUTE_NAME = "trial2_numeric_alpha_vector_qball_form_factor_hybrid_s7s8_registry"
NEXT_ROUTE = "8.7.56.2251"
FOLLOWUP_ROUTE_NAME = (
    "trial2_numeric_alpha_vector_qball_form_factor_hybrid_selected_farther_audit"
)
FOLLOWUP_ROUTE = "8.7.56.2255"
...
def build_formulae() -> dict[str, str]:
    """Return formulas used in the hybrid farther continuation audit."""
    return {
        "retained_lattice": "delta_q^(n,m)=delta_q,base^(box)+m_n Delta_box",
        "first_shot": "M_7(x)=a_7 x+b_7, C_7(x)=c_7 x+d_7, E_7(q)=A_7 q^{-nu_7}",
        "fastlane_scan": "Extend the promoted seventh prefix until monitor error first exceeds the retained tolerances.",
        "reserve_fallback": "M_8(x)=a_8 x+b_8, C_8(x)=c_8 x+d_8, E_8(q)=A_8 q^{-nu_8} fitted only after the seventh first shot fails on the full farther window.",
    }
...
def main() -> None:
    ...
    inventory_ready = bool(
        prior_registry_summary["gate_b_seventh_piecewise_reactivation_selected"]
    )
    ...
    rows = [
        base.sign_base.row(
            ...
            "The hybrid farther branch starts only after the seventh route has already been promoted through harmonic 1376256.",
        ),
        base.sign_base.row(
            "same_seventh_holdout_max_mismatch_abs_error",
            "pass" if seventh_holdout_mismatch <= base.MISMATCH_TOL else "reject",
            "same seventh first-shot holdout max mismatch abs error through harmonic 1409024",
            seventh_holdout_mismatch,
            "The accelerated first shot trusts the promoted seventh segment only if the initial quartet stays inside the retained mismatch tolerance.",
        ),
        base.sign_base.row(
            "same_seventh_holdout_max_correlation_abs_error",
            "pass" if seventh_holdout_corr <= base.CORRELATION_TOL else "reject",
            "same seventh first-shot holdout max correlation abs error through harmonic 1409024",
        ...
        base.sign_base.row(
            ...
            "same seventh full farther-window monitor max mismatch abs error through harmonic 1474560",
        ...
        base.sign_base.row(
            ...
            "same seventh full farther-window monitor max correlation abs error through harmonic 1474560",
            seventh_monitor_corr,
            "This correlation error is the current blocker for pushing the promoted seventh segment blindly across the entire farther branch.",
        ),
        base.sign_base.row(
            "same_seventh_supported",
            "pass" if same_seventh_supported else "reject",
            "same seventh first-shot continuation through harmonic 1474560 supported",
        ...
        base.sign_base.row(
            ...
            "reserve eighth holdout max mismatch abs error through harmonic 1441792",
            (
        ...
        base.sign_base.row(
            ...
            "reserve eighth holdout max correlation abs error through harmonic 1441792",
        ...
        base.sign_base.row(
            ...
            "reserve eighth monitor max mismatch abs error through harmonic 1474560",
            (
        ...
        base.sign_base.row(
            ...
            "reserve eighth monitor max correlation abs error through harmonic 1474560",
        ...
        base.sign_base.row(
            ...
            "After the accelerated same-seventh first shot fails, the reserve eighth exact fallback becomes the honest next surface only if it passes holdout and monitor windows.",
        ),
    ]
    ...
    summary = {
        ...
        "gate_a_same_seventh_piecewise_validation_to_1474560_retained": gate_a,
    ...
    declaration_payload = base.sign_base.payload(
        ...
        summary,
        {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_fast_declared",
        ...
        },
        {
            ...
            "hits": {
                "status_branch_hit": base.find_line(status_text, "8.7.56.2247"),
                "roadmap_branch_hit": base.find_line(roadmap_text, ".2247-.2250"),
                "current_problem_hit": base.find_line(current_problem_text, "8.7.56.2247"),
                "current_status_hit": base.find_line(current_status_text, "8.7.56.2247"),
                "unified_roadmap_hit": base.find_line(unified_text, ".2247-.2250"),
                "long_roadmap_hit": base.find_line(long_text, ".2247-.2250"),
                "part5_hit": base.find_line(part5_text, ".2247-.2250"),
            },
        },
    )
    ...
    route_payload = {
        ...
        "rows": [
            base.sign_base.row(
                ...
                1.0,
                "The hybrid farther audit is only honest if the official status already points to the promoted seventh branch that it accelerates.",
            ),
            base.sign_base.row(
                "roadmap_synced",
                "pass",
        ...
        "summary": summary,
        "decision": {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_fast_route_synced",
            "branch_completed": True,
            "next_required_artifacts": [NEXT_ROUTE_NAME],
        },
