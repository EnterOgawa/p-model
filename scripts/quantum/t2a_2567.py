#!/usr/bin/env python3
"""Generate 8.7.56.2567-.2570 updated-pack blind-vector refresh audit artifacts."""
...
NEXT_STEPS_CACHE = build_metrics_paths(
    PUBLIC_OUT,
    build_compact_artifact_stem(
        "8.7.56.2511-2514",
...
def build_formulae() -> dict[str, str]:
    ...
    return {
        "step_d": "After low-order closeout, evaluate F_vector(q) at q = 0, q = q_theory, and q = m_0",
        "alpha_vector": "alpha_vector(q) = F_vector(q)^2 / (4 pi)",
        "decision_focus": "preserve F_vector(0)=1, improve the retained 1.9% scalar residual at q_theory, and compare against the q = m_0 checkpoint",
        "refresh_order": "low-order J_eff^0 closeout -> blind-vector refresh -> residual-origin refresh",
    }
...
def main() -> None:
    ...
    next_steps_source = NEXT_STEPS if NEXT_STEPS.exists() else NEXT_STEPS_CACHE
    ...
    next_steps_text = sign_base.read_text(next_steps_source)
    ...
    updated_pack_low_order_jeff0_closeout_surface_retained = bool(
        prior_summary["gate_a_updated_pack_low_order_jeff0_closeout_surface_explicit"]
    )
    ...
    blind_vector_alpha_hit = sign_base.hit(next_steps_text, r"\alpha_{\rm vector}(q)=")
    if blind_vector_alpha_hit is None:
        blind_vector_alpha_hit = sign_base.hit(next_steps_text, "alpha_vector(q) =")

    blind_vector_alpha_target_explicit = bool(blind_vector_alpha_hit is not None)
    ...
    updated_pack_blind_vector_refresh_target_surface_explicit = bool(
        ...
        and updated_pack_low_order_jeff0_closeout_surface_retained
    ...
    rows = [
        sign_base.row(
            ...
            "The low-order closeout gate already promoted blind-vector refresh as the next honest downstream lane.",
        ),
        sign_base.row(
            "updated_pack_low_order_jeff0_closeout_surface_retained",
            "pass" if updated_pack_low_order_jeff0_closeout_surface_retained else "reject",
            "updated-pack low-order J_eff^0 closeout surface retained",
            sign_base.truth(updated_pack_low_order_jeff0_closeout_surface_retained),
            "The blind-vector lane is now explicitly downstream of the retained low-order closeout surface rather than the older refresh-only ordering.",
        ),
        sign_base.row(
            ...
            "The next-steps pack states blind-vector computation as its own named Step D rather than an implicit future remark.",
        ),
        sign_base.row(
            ...
            "The same note keeps blind evaluation conditional on the unresolved theorem stack instead of pretending that direct computation is already open.",
        ),
        sign_base.row(
        ...
        sign_base.row(
            ...
            "The note keeps alpha_vector(q) as the comparison quantity instead of a wording-only proxy.",
        ),
        sign_base.row(
            ...
            "The target remains improvement over the retained 1.9% scalar residual rather than a target-fit rewrite.",
        ),
        sign_base.row(
            "updated_pack_blind_vector_refresh_target_surface_explicit",
        ...
        sign_base.row(
            ...
            "The theorem stack still lacks the exact low-order formula, so Step D remains an ordered followup rather than an admissible direct computation.",
        ),
        sign_base.row(
            "vector_form_factor_exact_computation_ready_under_current_pack",
    ...
    summary = {
        ...
        "updated_pack_low_order_jeff0_closeout_surface_retained": updated_pack_low_order_jeff0_closeout_surface_retained,
    ...
    declaration_payload = sign_base.payload(
        ...
        STEP_NAME + " declaration gate",
        {
            "source_files": {
                ...
                "next_steps": sign_base.display_path(next_steps_source),
            },
            "routes": {
        ...
        },
        {
            ...
            "hits": {
                ...
                "long_roadmap_hit": sign_base.hit(long_text, ".2559-.2562"),
                ...
                "alpha_hit": blind_vector_alpha_hit,
