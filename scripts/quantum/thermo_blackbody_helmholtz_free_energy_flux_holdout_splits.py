"""
目的: 黒体 holdout 監査の helmholtz free energy flux 指標図を再生成する。
...
def main() -> None:
    ...
    helmholtz_flux_coeff_abs = stefan_boltzmann_sigma / 3.0

    temperature_grid_k = np.logspace(0.0, 4.0, 80)
    free_energy_flux = -helmholtz_flux_coeff_abs * (temperature_grid_k**4)
    free_energy_flux_sigma = np.maximum(1e-12, 1e-6 * np.abs(free_energy_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, free_energy_flux, train_idx)
        flux_pred_abs = (10**log10_coeff) * (temperature_grid_k**exponent)
        flux_pred = -np.abs(flux_pred_abs)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=free_energy_flux, y_pred=flux_pred, sigma=free_energy_flux_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=free_energy_flux, y_pred=flux_pred, sigma=free_energy_flux_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, free_energy_flux, train_idx, exponent=4.0)
        flux_pred_fixed = -np.abs(coeff_fixed_abs * (temperature_grid_k**4.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=free_energy_flux,
            y_pred=flux_pred_fixed,
            sigma=free_energy_flux_sigma,
            param_count=1,
            is_train=True,
        )
        ...
        models["fixed_exponent_4_abs_fit"] = {
            "params": {"abs_coeff": float(coeff_fixed_abs), "exponent": 4.0, "sign": -1.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_free_energy_flux_holdout_splits.csv"
    ...
    for sp in split_results:
        ...
        model_fixed = models.get("fixed_exponent_4_abs_fit") if isinstance(models, dict) else None
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 4 abs fit (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody Helmholtz free-energy flux holdout: J_f(T)=(c/4)f(T)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_free_energy_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_free_energy_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "helmholtz_flux_abs_coeff": helmholtz_flux_coeff_abs,
                },
                "assumptions": {
                    "sign_convention": "J_f(T) is negative for photon gas; model fit is applied to abs(J_f) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for blackbody Helmholtz free-energy flux J_f(T)=(c/4)f(T)=-(sigma/3)T^4.",
