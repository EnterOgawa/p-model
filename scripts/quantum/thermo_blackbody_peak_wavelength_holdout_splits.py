"""
目的: 黒体 holdout 監査の peak wavelength 指標図を再生成する。
...
# 関数: `_wien_peak_x` の入出力契約と処理意図を定義する。

def _wien_peak_x() -> float:
...
def main() -> None:
    ...
    lambda_sigma = np.maximum(1e-15, 1e-6 * lambda_peak_m)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, lambda_peak_m, train_idx)
        lambda_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=lambda_peak_m, y_pred=lambda_pred, sigma=lambda_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=lambda_peak_m, y_pred=lambda_pred, sigma=lambda_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, lambda_peak_m, train_idx, exponent=-1.0)
        lambda_pred_fixed = coeff_fixed * (temperature_grid_k**-1.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=lambda_peak_m,
            y_pred=lambda_pred_fixed,
            sigma=lambda_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_peak_wavelength_holdout_splits.csv"
    ...
    for sp in split_results:
        ...
        model_fixed = models.get("fixed_exponent_-1_fit") if isinstance(models, dict) else None
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody peak wavelength holdout: λ_max T = b (Wien)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_peak_wavelength_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_peak_wavelength_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody peak wavelength from the Planck B_λ maximum (Wien displacement law).",
                    "The root x solves 5(1-exp(-x))=x; b = h c / (k_B x).",
                ],
            },
            ensure_ascii=False,
            indent=2,
