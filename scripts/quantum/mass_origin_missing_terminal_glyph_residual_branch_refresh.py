...
# -*- coding: utf-8 -*-
"""
mass_origin_missing_terminal_glyph_residual_branch_refresh.py

Step 8.7.55.2.234:
Reinject the residual missing-terminal-glyph route results into the blocked
...
PRIOR_REFRESH_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_terminal_atom_residual_branch_refresh_metrics.json"
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_terminal_glyph_residual_branch_refresh_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_missing_terminal_glyph_residual_branch_refresh_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.234"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Refresh blocked second-route handoff eligibility after missing-terminal-glyph residual retries.")
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    target_source_kind = "shell_anchor_missing_terminal_glyph_bridge" if target_available else None
    ...
    remaining_blockers = _ordered_unique(
        [
            str(shell_anchor_retry_summary.get("shell_anchor_missing_terminal_glyph_nonclosure_reason_or_none") or ""),
            str(explicit_mapping_retry_summary.get("explicit_mapping_missing_terminal_glyph_nonclosure_reason_or_none") or ""),
        ]
    )

    rows: List[Dict[str, Any]] = [
        {
            "row_id": "missing_terminal_glyph_residual_branch_refresh_complete",
            "status": "pass",
            "metric": "missing-terminal-glyph residual branch refresh complete",
            "value": 1.0,
            "note": "This refresh reinjects terminal-glyph residual retries into the blocked second-route stack and refreezes .83-.84 handoff eligibility.",
        },
        {
            "row_id": "hand_off_to_8_7_55_2_83",
            "status": "pass" if handoff else "reject",
            "metric": "handoff to 8.7.55.2.83-.84 allowed after missing-terminal-glyph residual refresh",
            "value": 1.0 if handoff else 0.0,
            "note": "Handoff to the discrete-spectrum pilot is now allowed." if handoff else "Handoff remains blocked because missing-terminal-glyph residual reinjection did not reopen the branch.",
        },
        {
            "row_id": "missing_terminal_glyph_residual_branch_refresh_source_level_blocker_count",
            "status": "inventory",
            "metric": "remaining source-level blocker count after missing-terminal-glyph residual refresh",
    ...
    return {
        ...
        "phase": {"phase": 8, "step": step_tag, "name": "missing-terminal-glyph residual branch refresh / handoff"},
        "inputs": {
            "mass_origin_missing_terminal_atom_residual_branch_refresh_json": _relative_str(PRIOR_REFRESH_JSON),
        ...
        "decision": {
            "overall_status": (
                "missing_terminal_glyph_residual_branch_refresh_reopen_ready"
                if handoff
                else "missing_terminal_glyph_residual_branch_refresh_still_blocked"
            ),
