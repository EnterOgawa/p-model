...
# -*- coding: utf-8 -*-
"""
mass_origin_boundary_radius_source_inventory.py

Step 8.7.55.2.406:
Inventory the source candidates that would be required to lift the surviving
shell-quantization family into a boundary radius or domain proxy for the
...
Outputs:
  ...
  - output/public/quantum/mass_origin_boundary_radius_source_inventory_rows.csv
"""
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_boundary_radius_source_inventory_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_boundary_radius_source_inventory_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.406"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Inventory source candidates for the boundary-radius proxy route.")
...
def _build_rows(
    *,
    ...
    rows: List[Dict[str, Any]] = [
        {
            "row_id": "boundary_radius_source_inventory_complete",
            "status": "pass",
            "metric": "boundary-radius source inventory complete",
            "value": 1.0,
            "note": "This step inventories which public-canonical ingredients are still missing before shell quantization can define a cavity radius or domain proxy.",
        },
        {
            "row_id": "boundary_radius_required_source_count",
            "status": "pass",
            "metric": "required source count for boundary-radius proxy route",
            "value": float(len(required_sources)),
            "note": f"Required sources: {required_sources}.",
        },
        {
            "row_id": "boundary_radius_present_source_count",
            "status": "pass",
            "metric": "present source count for boundary-radius proxy route",
            "value": float(len(present_sources)),
            "note": f"Present sources: {present_sources}.",
        },
        {
            "row_id": "boundary_radius_missing_source_count",
            "status": "watch" if missing_sources else "pass",
            "metric": "missing source count for boundary-radius proxy route",
    ...
    for source_name in required_sources:
        ...
        rows.append(
            {
                "row_id": f"boundary_radius_source_{source_name}",
    ...
    rows.append(
        {
            "row_id": "boundary_radius_first_route_to_close",
            "status": "watch" if first_route_to_close_or_none else "reject",
            "metric": "first residual source to close for boundary-radius proxy route",
            ...
            "note": (
                ...
                else "No residual source could be prioritized from the boundary-radius proxy inventory."
            ),
        }
    )
    return rows
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    for path in (ROUTE_CONTRACT_JSON, CAVITY_SOURCE_JSON, SHELL_JSON, NOTE_MD):
        _require_path(path)
    ...
    reflective_line = _find_first_line(NOTE_MD, "Dirichlet/Neumann")

    route_contract_summary = route_contract.get("summary", {})
    cavity_source_summary = cavity_source.get("summary", {})
    cavity_source_evidence = cavity_source.get("evidence", {})
    ...
    if "shell_quantization_kappa_row" in cavity_source_evidence:
        present_sources.append("shell_quantization_fit_kappa_row")

    if "shell_quantization_kz_over_kn_row" in cavity_source_evidence:
    ...
    if reflective_line["line"] is not None:
    ...
    first_route_to_close_or_none = "shell_quantization_to_domain_relation" if "shell_quantization_to_domain_relation" in missing_sources else None
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "boundary-radius source inventory",
        },
        "inputs": {
        ...
        "intent": "Inventory the source candidates required to lift shell quantization into a boundary radius or domain proxy for the geometric reflective-boundary route.",
        "formulas": {
            "inventory_rule": "the boundary-radius proxy route can close only after the public pack contains shell-family rows, a geometric domain symbol, reflective-boundary wording, and a same-sector relation that maps shell quantization into a cavity domain",
            "current_absence": "the current public pack documents shell quantization and reflective boundary conditions, but it still does not expose a relation from shell-family rows to a cavity radius or domain",
        },
        "rows": rows,
        "summary": {
            "required_boundary_radius_sources": required_sources,
            "present_boundary_radius_sources": present_sources,
            "missing_boundary_radius_sources": missing_sources,
            "first_route_to_close_or_none": first_route_to_close_or_none,
            "boundary_radius_source_inventory_ready": True,
        },
        "decision": {
            "overall_status": "boundary_radius_source_inventory_frozen",
        ...
        "evidence": {
            ...
            "shell_quantization_family_row": cavity_source_evidence.get("shell_quantization_family_row"),
            "shell_quantization_kappa_row": cavity_source_evidence.get("shell_quantization_kappa_row"),
            "shell_quantization_kz_over_kn_row": cavity_source_evidence.get("shell_quantization_kz_over_kn_row"),
            "mass_origin_note_domain_symbol_line": domain_line,
            "mass_origin_note_reflective_boundary_line": reflective_line,
        },
    }
