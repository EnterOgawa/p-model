#!/usr/bin/env python3
"""Generate 8.7.56.2255-.2258 hybrid seventh/eighth ultra-farther artifacts."""
...
PRIOR_AUDIT = (
    base.PUBLIC_OUT
    / "q_8_7_56_2247_2250_harmonic_hybrid_s7_s8_fast_declaration_gate_metrics.json"
)
PRIOR_REGISTRY = (
    base.PUBLIC_OUT
    / "q_8_7_56_2251_2254_harmonic_hybrid_s7_s8_registry_declaration_gate_metrics.json"
)

STEP_TAG = "8.7.56.2255-2258"
STEP_NAME = (
    "Trial-2 numeric alpha vector Q-ball form-factor hybrid seventh/eighth "
    "ultra-farther audit"
)
STEM = build_compact_artifact_stem(STEP_TAG, "harmonic_hybrid_s7_s8_ultra_fast", prefix="q")
base.STEM = STEM

PRIOR_CLASS = (
    "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_retained_1474560_next"
)
BRANCH_CLASS = "vector_qball_form_factor_boundary_bulk_lattice_hybrid_s7_s8_ultra_fast_gate"
NEXT_ROUTE_NAME = "trial2_numeric_alpha_vector_qball_form_factor_hybrid_s7s8_ultra_registry"
NEXT_ROUTE = "8.7.56.2259"
FOLLOWUP_ROUTE_NAME = (
    "trial2_numeric_alpha_vector_qball_form_factor_hybrid_selected_extreme_ultra_farther_audit"
)
FOLLOWUP_ROUTE = "8.7.56.2263"
...
def build_formulae() -> dict[str, str]:
    """Return formulas used in the hybrid ultra-farther continuation audit."""
    return {
        "retained_lattice": "delta_q^(n,m)=delta_q,base^(box)+m_n Delta_box",
        "first_shot": "M_7(x)=a_7 x+b_7, C_7(x)=c_7 x+d_7, E_7(q)=A_7 q^{-nu_7}",
        "fastlane_scan": "Extend the retained seventh prefix until monitor error first exceeds the retained tolerances on the ultra-farther branch.",
        "reserve_fallback": "M_8(x)=a_8 x+b_8, C_8(x)=c_8 x+d_8, E_8(q)=A_8 q^{-nu_8} fitted only after the retained seventh first shot fails on the full ultra-farther window.",
    }
...
def main() -> None:
    """Execute the hybrid seventh/eighth ultra-farther continuation audit."""
    for path in (
        base.STATUS,
        base.ROADMAP,
        base.AI_CONTEXT,
    ...
    inventory_ready = bool(
        prior_registry_summary["gate_a_same_seventh_piecewise_validation_to_1474560_retained"]
    )
    ...
    seventh_m_intercept = float(prior_audit_summary["seventh_post_break_piecewise_mismatch_intercept"])
    seventh_c_slope = float(prior_audit_summary["seventh_post_break_piecewise_correlation_slope"])
    seventh_c_intercept = float(prior_audit_summary["seventh_post_break_piecewise_correlation_intercept"])
    seventh_rec_exp = float(prior_audit_summary["seventh_post_break_reconstruction_decay_exponent"])
    seventh_rec_pref = float(prior_audit_summary["seventh_post_break_reconstruction_decay_prefactor"])
    ...
    reserve_executed = bool(not same_seventh_supported)
    ...
    gate_a = bool(same_seventh_supported)
    gate_b = bool((not gate_a) and eighth_supported)
    ...
    rows = [
        base.sign_base.row(
            ...
            "hybrid ultra-farther inventory ready",
            base.sign_base.truth(inventory_ready),
            "The hybrid ultra-farther branch starts only after the seventh route has already been retained through harmonic 1474560.",
        ),
        base.sign_base.row(
            "same_seventh_holdout_max_mismatch_abs_error",
            "pass" if seventh_holdout_mismatch <= base.MISMATCH_TOL else "reject",
            "same seventh first-shot holdout max mismatch abs error through harmonic 1507328",
        ...
        base.sign_base.row(
            ...
            "pass" if seventh_holdout_corr <= base.CORRELATION_TOL else "reject",
            "same seventh first-shot holdout max correlation abs error through harmonic 1507328",
        ...
        base.sign_base.row(
            ...
            "The first failing prefix marks the natural ceiling of the accelerated same-seventh route on the current ultra-farther branch.",
        ),
        base.sign_base.row(
            "same_seventh_full_monitor_max_mismatch_abs_error",
            "pass" if seventh_monitor_mismatch <= base.MISMATCH_TOL else "reject",
            "same seventh full ultra-farther-window monitor max mismatch abs error through harmonic 1572864",
            seventh_monitor_mismatch,
            "The full ultra-farther window is the honest first-shot gate; once it fails, the exact reserve fallback is allowed to run.",
        ),
        base.sign_base.row(
            "same_seventh_full_monitor_max_correlation_abs_error",
            "pass" if seventh_monitor_corr <= base.CORRELATION_TOL else "reject",
            "same seventh full ultra-farther-window monitor max correlation abs error through harmonic 1572864",
            seventh_monitor_corr,
            "This correlation error is the current blocker for pushing the retained seventh segment blindly across the entire ultra-farther branch.",
        ),
        base.sign_base.row(
            "same_seventh_supported",
            "pass" if same_seventh_supported else "reject",
            "same seventh first-shot continuation through harmonic 1572864 supported",
            base.sign_base.truth(same_seventh_supported),
            "The accelerated route keeps the same seventh segment only if the entire ultra-farther window stays inside tolerance.",
        ),
        base.sign_base.row(
            ...
            "The exact fallback is triggered only after the accelerated same-seventh first shot fails on the full ultra-farther window.",
        ),
        base.sign_base.row(
            ...
            "reserve eighth holdout max mismatch abs error through harmonic 1540096",
            (
                float(eighth_holdout_mismatch)
                if eighth_holdout_mismatch is not None
                else -1.0
            ),
        ...
        base.sign_base.row(
            ...
            "reserve eighth holdout max correlation abs error through harmonic 1540096",
        ...
        base.sign_base.row(
            ...
            "reserve eighth monitor max mismatch abs error through harmonic 1572864",
            (
                float(eighth_monitor_mismatch)
                if eighth_monitor_mismatch is not None
                else -1.0
            ),
            "This is the decisive ultra-farther monitor for the reserve eighth exact fallback.",
        ),
        base.sign_base.row(
            ...
            "reserve eighth monitor max correlation abs error through harmonic 1572864",
        ...
        base.sign_base.row(
            ...
            "After the accelerated same-seventh first shot fails, the reserve eighth exact fallback becomes the honest next surface only if it passes holdout and monitor windows on the ultra-farther branch.",
        ),
    ]
    ...
    declaration_payload = base.sign_base.payload(
        ...
        summary,
        {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_ultra_fast_declared",
        ...
        },
        {
            ...
            "hits": {
                "status_branch_hit": base.find_line(status_text, "8.7.56.2255"),
                "roadmap_branch_hit": base.find_line(roadmap_text, ".2255-.2258"),
                "current_problem_hit": base.find_line(current_problem_text, "8.7.56.2255"),
                "current_status_hit": base.find_line(current_status_text, "8.7.56.2255"),
                "unified_roadmap_hit": base.find_line(unified_text, ".2255-.2258"),
                "long_roadmap_hit": base.find_line(long_text, ".2255-.2258"),
                "part5_hit": base.find_line(part5_text, ".2255-.2258"),
            },
        },
    )
    ...
    route_payload = {
        ...
        "rows": [
            base.sign_base.row(
                ...
                1.0,
                "The hybrid ultra-farther audit is only honest if the official status already points to the retained seventh branch that it accelerates.",
            ),
            base.sign_base.row(
                "roadmap_synced",
                "pass",
        ...
        "summary": summary,
        "decision": {
            "overall_status": "vector_qball_form_factor_hybrid_s7_s8_ultra_fast_route_synced",
            "branch_completed": True,
            "next_required_artifacts": [NEXT_ROUTE_NAME],
        },
