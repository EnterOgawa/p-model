...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_continuity_audit.py

Step 8.7.55.2.27:
Freeze a downstream continuity audit for the mass-origin blocked interface
stack, so later readers can detect whether the full blocked-interface manifest
changed as a single unit instead of reopening each blocked interface manually.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_interface_stack_continuity_rows.csv

Assumptions:
  - The blocked interface-stack manifest is the single public canonical
    summary of the current mass-origin blocked interface layer.
  - This audit is downstream only; it does not change blocked-state logic and
    only records whether the blocked interface stack changed as a whole.
"""
...
MANIFEST_ROWS_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_manifest_rows.csv"
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_continuity_metrics.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_continuity_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.27"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze a downstream continuity audit for the mass-origin blocked interface stack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _derive_continuity_watchpack(
    *,
    ...
    # 条件分岐: 前段条件が不成立で、`continuity_state_changed` を追加評価する。
        ...
        update_event_type = "blocked_interface_stack_state_changed"
    ...
    if continuity_state_changed:
        next_action = "rerun_blocked_interface_stack_readers_now"
    # 条件分岐: 前段条件が不成立で、`logic_state.get(\"keep_mass_origin_branch_blocked\", True)` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_interface_stack_continuity"
    else:
    ...
    return {
        ...
        "note": (
            "Event counter increments only when the logical blocked interface-stack state changes. "
...
def _build_rows(logic_state: Dict[str, Any], watchpack: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        {
            "row_id": "blocked_interface_stack_manifest_signature_present",
            "status": "pass" if str(logic_state.get("manifest_signature_sha256") or "") else "reject",
            "metric": "blocked interface-stack manifest signature is present",
            "value": 1.0 if str(logic_state.get("manifest_signature_sha256") or "") else 0.0,
            "note": "Continuity audit requires a stable manifest signature from the blocked interface-stack manifest.",
        },
        {
            "row_id": "blocked_interface_stack_rows_sync",
            "status": "pass" if bool(logic_state.get("rows_csv_matches_manifest", False)) else "reject",
            "metric": "interface-stack rows CSV count matches manifest summary count",
        ...
        },
        {
            ...
            "metric": "blocked-state detail propagated into blocked interface-stack continuity",
        ...
        {
            "row_id": "blocked_interface_stack_complete",
            "status": "pass" if bool(logic_state.get("interface_stack_complete", False)) else "reject",
            "metric": "blocked interface stack remains complete",
            "value": 1.0 if bool(logic_state.get("interface_stack_complete", False)) else 0.0,
            "note": "Continuity audit assumes the blocked interface stack remains complete before downstream readers rely on it.",
        },
        {
            "row_id": "mass_origin_branch_blocked_in_stack_manifest",
            "status": "blocked" if bool(logic_state.get("keep_mass_origin_branch_blocked", True)) else "pass",
            "metric": "interface-stack manifest keeps the mass-origin branch blocked",
        ...
        },
        {
            ...
            "note": "The current blocked interface stack still inherits a settled hold monitor state.",
        },
        {
            "row_id": "terminal_stack_status_locked_no_change_hold",
            "status": "pass" if str(logic_state.get("terminal_stack_status", "")) == "watch:blocked_interface_stack_locked_no_change_hold" else "watch",
            "metric": "terminal stack status remains locked on no-change hold",
            "value": 1.0 if str(logic_state.get("terminal_stack_status", "")) == "watch:blocked_interface_stack_locked_no_change_hold" else 0.0,
            "note": f"Current terminal_stack_status = {logic_state.get('terminal_stack_status')}.",
        },
        {
            "row_id": "blocked_interface_stack_continuity_state_settled",
            "status": "pass" if state_settled else "watch",
            "metric": "blocked interface-stack logical state is settled",
            "value": 1.0 if state_settled else 0.0,
            "note": (
                "Continuity state is unchanged from the previous interface-stack continuity snapshot."
                if state_settled
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    current_input_signatures = {
        "blocked_interface_stack_manifest_json": _file_signature(MANIFEST_JSON),
        "blocked_interface_stack_manifest_rows_csv": _file_signature(MANIFEST_ROWS_CSV),
    }
    ...
    if has_reject:
        ...
        decision_text = "blocked_interface_stack_sync_broken"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(watchpack.get(\"continuity_state_changed\", False))` を追加評価する。
        ...
        overall_status = "watch"
        decision_text = "blocked_interface_stack_state_changed"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack continuity audit",
        },
        "inputs": {
            "blocked_interface_stack_manifest_json": _rel(MANIFEST_JSON),
            "blocked_interface_stack_manifest_rows_csv": _rel(MANIFEST_ROWS_CSV),
        },
        "intent": "Track whether the mass-origin blocked interface stack changes as a single public canonical unit and keep downstream readers aligned with the current blocked state.",
        "summary": {
            ...
            "terminal_stack_status": str(logic_state.get("terminal_stack_status", "")),
            "terminal_stack_signature_sha256": str(logic_state.get("terminal_stack_signature_sha256", "")),
        ...
        "decision": {
            ...
            "interface_stack_complete": bool(logic_state.get("interface_stack_complete", False)),
