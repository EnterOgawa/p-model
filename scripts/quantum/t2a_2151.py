#!/usr/bin/env python3
"""Generate 8.7.56.2151-.2154 second post-break farther continuation artifacts."""
...
PRIOR_LATTICE_AUDIT = build_metrics_paths(
    PUBLIC_OUT,
...
NEXT_ROUTE = "8.7.56.2155"
FOLLOWUP_ROUTE_NAME = (
    "trial2_numeric_alpha_vector_qball_form_factor_second_post_break_"
    "piecewise_ultra_farther_continuation_audit"
)
FOLLOWUP_ROUTE = "8.7.56.2159"
...
SECOND_HOLDOUT = FARTHER_BANDS[:4]
SECOND_MONITOR = FARTHER_BANDS[4:]
THIRD_FIT = FARTHER_BANDS[:4]
THIRD_HOLDOUT = FARTHER_BANDS[4:8]
THIRD_MONITOR = FARTHER_BANDS[8:]
...
def build_formulae() -> dict[str, str]:
    ...
    return {
        ...
        "third_piecewise": "M_3(x)=a_3 x+b_3, C_3(x)=c_3 x+d_3, E_3(q)=A_3 q^{-nu_3} fitted on 294913..327680",
        "selection_rule": "A third piecewise surface is inadmissible while the inherited second segment already passes both farther holdout and monitor windows.",
    }
...
def main() -> None:
    """Execute the second post-break farther continuation audit."""
    for path in (
        STATUS,
        ROADMAP,
        AI_CONTEXT,
    ...
    prior_lattice_summary = sign_base.read_json(PRIOR_LATTICE_AUDIT)["summary"]
    prior_registry_summary = sign_base.read_json(PRIOR_REGISTRY)["summary"]
    inventory_ready = bool(
        prior_registry_summary["gate_b_second_piecewise_validation_to_294912_retained"]
    )
    ...
    theorem_lattice_base = float(prior_lattice_summary["theorem_lattice_base_over_m0"])
    theorem_lattice_step = float(prior_lattice_summary["bulk_delta_r_over_m0"])
    ...
    second_monitor_slice = slice(len(SECOND_HOLDOUT), len(FARTHER_BANDS))
    second_farther_holdout_max_mismatch_abs_error = max_abs_error(
        mismatches[second_holdout_slice],
        second_m_pred[second_holdout_slice],
    )
    second_farther_holdout_max_correlation_abs_error = max_abs_error(
        correlations[second_holdout_slice],
        second_c_pred[second_holdout_slice],
    )
    second_farther_holdout_max_reconstruction_abs_error = max_abs_error(
        recon_errors[second_holdout_slice],
        second_r_pred[second_holdout_slice],
    )
    second_farther_monitor_max_mismatch_abs_error = max_abs_error(
        mismatches[second_monitor_slice],
        second_m_pred[second_monitor_slice],
    )
    second_farther_monitor_max_correlation_abs_error = max_abs_error(
        correlations[second_monitor_slice],
        second_c_pred[second_monitor_slice],
    )
    second_farther_monitor_max_reconstruction_abs_error = max_abs_error(
        recon_errors[second_monitor_slice],
        second_r_pred[second_monitor_slice],
    )
    same_second_piecewise_farther_continuation_supported = bool(
        second_farther_holdout_max_mismatch_abs_error <= MISMATCH_TOL
        and second_farther_holdout_max_correlation_abs_error <= CORRELATION_TOL
        and second_farther_holdout_max_reconstruction_abs_error <= RECON_TOL
        and second_farther_monitor_max_mismatch_abs_error <= MISMATCH_TOL
        and second_farther_monitor_max_correlation_abs_error <= CORRELATION_TOL
        and second_farther_monitor_max_reconstruction_abs_error <= RECON_TOL
    )
    second_post_break_piecewise_validation_to_393216_supported = bool(
        same_second_piecewise_farther_continuation_supported
    )
    ...
    third_post_break_piecewise_validation_to_393216_supported = bool(
    ...
    third_post_break_piecewise_surface_admissible_now = bool(
        (not same_second_piecewise_farther_continuation_supported)
        and third_post_break_piecewise_validation_to_393216_supported
    )
    exact_global_farther_second_post_break_theorem_available = False
    ...
    rows = [
        sign_base.row(
            ...
            "second post-break farther inventory ready",
            sign_base.truth(inventory_ready),
            "The farther continuation starts only after the second segment has already been retained through harmonic 294912.",
        ),
        sign_base.row(
            "second_farther_holdout_max_mismatch_abs_error",
            "pass" if second_farther_holdout_max_mismatch_abs_error <= MISMATCH_TOL else "reject",
            "same second piecewise farther holdout max mismatch abs error through harmonic 327680",
            second_farther_holdout_max_mismatch_abs_error,
            "The inherited second segment survives only if the next quartet stays inside the retained mismatch tolerance.",
        ),
        sign_base.row(
            "second_farther_holdout_max_correlation_abs_error",
            "pass" if second_farther_holdout_max_correlation_abs_error <= CORRELATION_TOL else "reject",
            "same second piecewise farther holdout max correlation abs error through harmonic 327680",
            second_farther_holdout_max_correlation_abs_error,
            "The sign-floor channel must confirm the same farther survival for the inherited second segment.",
        ),
        sign_base.row(
            "second_farther_monitor_max_mismatch_abs_error",
            "pass" if second_farther_monitor_max_mismatch_abs_error <= MISMATCH_TOL else "reject",
            "same second piecewise farther monitor max mismatch abs error through harmonic 393216",
            second_farther_monitor_max_mismatch_abs_error,
            "The farther monitor checks that the same second segment does not collapse immediately after the first rescued quartet.",
        ),
        sign_base.row(
            "second_farther_monitor_max_correlation_abs_error",
            "pass" if second_farther_monitor_max_correlation_abs_error <= CORRELATION_TOL else "reject",
            "same second piecewise farther monitor max correlation abs error through harmonic 393216",
            second_farther_monitor_max_correlation_abs_error,
            "The monitor condition must also hold on the sign-floor channel.",
        ),
        sign_base.row(
            "same_second_piecewise_farther_continuation_supported",
            "pass" if same_second_piecewise_farther_continuation_supported else "reject",
            "same second post-break piecewise farther continuation supported",
            sign_base.truth(same_second_piecewise_farther_continuation_supported),
            "No new surface is admissible while the inherited second segment still survives farther holdout and monitor windows.",
        ),
        sign_base.row(
            "third_post_break_piecewise_mismatch_slope",
            "watch",
            "third post-break exploratory mismatch slope",
            third_m_slope,
            "A third segment is computed only as a reserve diagnostic after the same second segment has already been tested on the farther window.",
        ),
        sign_base.row(
            "third_holdout_max_mismatch_abs_error",
            "pass" if third_holdout_max_mismatch_abs_error <= MISMATCH_TOL else "reject",
            "third post-break holdout max mismatch abs error through harmonic 360448",
            third_holdout_max_mismatch_abs_error,
        ...
        sign_base.row(
            ...
            "third post-break holdout max correlation abs error through harmonic 360448",
            third_holdout_max_correlation_abs_error,
        ...
        sign_base.row(
            ...
            "third post-break monitor max mismatch abs error through harmonic 393216",
            third_monitor_max_mismatch_abs_error,
        ...
        sign_base.row(
            ...
            "third post-break monitor max correlation abs error through harmonic 393216",
            third_monitor_max_correlation_abs_error,
        ...
        sign_base.row(
            ...
            "The retry gate blocks a third segment while the inherited second segment already passes the farther continuation audit.",
        ),
    ]
    ...
    summary = {
        ...
        "second_farther_holdout_max_mismatch_abs_error": second_farther_holdout_max_mismatch_abs_error,
        "second_farther_holdout_max_correlation_abs_error": second_farther_holdout_max_correlation_abs_error,
        "second_farther_holdout_max_reconstruction_abs_error": second_farther_holdout_max_reconstruction_abs_error,
        "second_farther_monitor_max_mismatch_abs_error": second_farther_monitor_max_mismatch_abs_error,
        "second_farther_monitor_max_correlation_abs_error": second_farther_monitor_max_correlation_abs_error,
        "second_farther_monitor_max_reconstruction_abs_error": second_farther_monitor_max_reconstruction_abs_error,
        "same_second_piecewise_farther_continuation_supported": same_second_piecewise_farther_continuation_supported,
        "second_post_break_piecewise_validation_to_393216_supported": second_post_break_piecewise_validation_to_393216_supported,
        ...
        "third_post_break_piecewise_validation_to_393216_supported": third_post_break_piecewise_validation_to_393216_supported,
        "third_post_break_piecewise_surface_admissible_now": third_post_break_piecewise_surface_admissible_now,
        "exact_global_farther_second_post_break_theorem_available": exact_global_farther_second_post_break_theorem_available,
    ...
    declaration_payload = sign_base.payload(
        ...
        STEP_NAME + " declaration gate",
        {
            "source_files": {
                ...
                "prior_lattice_audit": sign_base.display_path(PRIOR_LATTICE_AUDIT),
        ...
        summary,
        {
            "overall_status": "vector_qball_form_factor_second_post_break_farther_declared",
        ...
        },
        {
            ...
            "hits": {
                "status_branch_hit": find_line(status_text, "8.7.56.2151"),
                "roadmap_branch_hit": find_line(roadmap_text, "8.7.56.2151-.2154"),
                "current_problem_hit": find_line(current_problem_text, "8.7.56.2151"),
                "current_status_hit": find_line(current_status_text, "8.7.56.2151"),
                "unified_roadmap_hit": find_line(unified_text, ".2147-.2150"),
                "long_roadmap_hit": find_line(long_text, ".2147-.2150"),
                "part5_hit": find_line(part5_text, ".2143-.2150"),
            },
        },
    )
    ...
    route_sync_rows = [
        sign_base.row(
            ...
            sign_base.truth(bool(find_line(status_text, "8.7.56.2151"))),
            "The farther continuation audit is only honest if the official status already points to the same second-segment route.",
        ),
        sign_base.row(
            "roadmap_synced",
            "pass",
            "ROADMAP sync target present",
            sign_base.truth(bool(find_line(roadmap_text, "8.7.56.2151-.2154"))),
            "The public roadmap must expose the same second post-break farther branch before route sync can proceed.",
        ),
        sign_base.row(
            "long_horizon_synced",
            "pass",
            "long-horizon roadmap sync target present",
            sign_base.truth(bool(find_line(long_text, ".2147-.2150"))),
            "The long-horizon roadmap must still expose the registry state before the farther continuation result is frozen.",
        ),
    ]
    route_sync_payload = sign_base.payload(
        ...
        summary,
        {
            "overall_status": "vector_qball_form_factor_second_post_break_farther_route_synced",
            "branch_completed": True,
            "next_required_artifacts": [NEXT_ROUTE_NAME],
        },
        {
