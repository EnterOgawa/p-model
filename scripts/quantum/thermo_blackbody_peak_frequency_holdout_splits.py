"""
目的: 黒体 holdout 監査の peak frequency 指標図を再生成する。
...
def main() -> None:
    ...
    x_peak = _wien_peak_x_frequency()
    nu_coeff = (boltzmann_k / planck_h) * x_peak

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    nu_peak_hz = nu_coeff * temperature_grid_k

    nu_sigma = np.maximum(1e-12, 1e-6 * nu_peak_hz)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, nu_peak_hz, train_idx)
        nu_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=nu_peak_hz, y_pred=nu_pred, sigma=nu_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=nu_peak_hz, y_pred=nu_pred, sigma=nu_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, nu_peak_hz, train_idx, exponent=1.0)
        nu_pred_fixed = coeff_fixed * temperature_grid_k
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=nu_peak_hz,
            y_pred=nu_pred_fixed,
            sigma=nu_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_peak_frequency_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody peak frequency holdout: ν_max ∝ T")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_peak_frequency_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_peak_frequency_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "wien_x_peak_frequency": x_peak,
                    "nu_coeff_Hz_per_K": nu_coeff,
                },
                "assumptions": {
                ...
                "notes": [
                    "Analytic holdout for blackbody peak frequency from the Planck B_ν maximum (Wien displacement law).",
                    "The root x solves 3(1-exp(-x))=x; ν_max = (k_B/h) x T.",
                ],
            },
            ensure_ascii=False,
            indent=2,
