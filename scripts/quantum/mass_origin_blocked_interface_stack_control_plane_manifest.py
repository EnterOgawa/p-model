...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_manifest.py
...
inspect the complete top-level control-plane layer without reopening each
...
Assumptions:
  - The control-plane pack is complete only if every current top-level
    control-plane artifact and the control-plane audit exist together.
...
STACK_MANIFEST_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_manifest.json"
STACK_CONTINUITY_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_continuity_metrics.json"
STACK_RERUN_POLICY_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_rerun_policy.json"
STACK_OPERATIONAL_CYCLE_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_operational_cycle.json"
CONTROL_PLANE_AUDIT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_metrics.json"
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_manifest.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_manifest_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.31"

ARTIFACT_SPECS = [
    {
        "artifact_id": "mass_origin_blocked_interface_stack_manifest",
        "role": "control_plane_input",
        "path": STACK_MANIFEST_JSON,
        "note": "Frozen manifest for the blocked interface stack.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_continuity",
        "role": "control_plane_input",
        "path": STACK_CONTINUITY_JSON,
        "note": "Continuity audit for the blocked interface stack.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_rerun_policy",
        "role": "control_plane_input",
        "path": STACK_RERUN_POLICY_JSON,
        "note": "Rerun/apply policy for the blocked interface stack.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_operational_cycle",
        "role": "control_plane_input",
        "path": STACK_OPERATIONAL_CYCLE_JSON,
        "note": "Operational-cycle decision for the blocked interface stack.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_audit",
        "role": "control_plane_audit",
        "path": CONTROL_PLANE_AUDIT_JSON,
        "note": "Top-level control-plane consistency audit across the current interface-stack pack.",
    },
]
        ...
        rows.append(
            {
                ...
                "status": "pass",
                "metric": f"{spec['role']} artifact present in control-plane pack",
                "value": 1.0,
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    terminal_row = next(
        row for row in rows if row["row_id"] == "mass_origin_blocked_interface_stack_control_plane_audit"
    )
    terminal_payload = _read_json(CONTROL_PLANE_AUDIT_JSON)
    ...
    continuity_payload = _read_json(STACK_CONTINUITY_JSON)
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane manifest",
        },
        "inputs": {
        ...
        "summary": {
            ...
            "terminal_control_plane_status": str(terminal_row.get("artifact_status", "")),
            "control_plane_signature_sha256": str(terminal_summary.get("control_plane_signature_sha256", "")),
            "continuity_status": str(_extract_artifact_status(continuity_payload)),
            "next_required_artifacts": next_required_artifacts,
        },
        "decision": {
            "overall_status": "blocked_interface_stack_control_plane_manifest_frozen",
            ...
            "next_required_artifacts": next_required_artifacts,
            "next_action": "wait_for_new_public_artifact_then_refresh_interface_stack_control_plane_manifest",
        },
        "evidence": {
            "terminal_control_plane_audit": {
                "path": _rel(CONTROL_PLANE_AUDIT_JSON),
                "phase_step": _extract_phase_step(terminal_payload),
                "artifact_status": str(terminal_row.get("artifact_status", "")),
            },
            "continuity_artifact": {
                "path": _rel(STACK_CONTINUITY_JSON),
