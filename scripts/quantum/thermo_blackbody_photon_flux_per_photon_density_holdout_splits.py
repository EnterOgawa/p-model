"""
目的: 黒体 holdout 監査の photon flux per photon density 指標図を再生成する。
...
# 関数: `_fit_log_powerlaw_abs` の入出力契約と処理意図を定義する。

def _fit_log_powerlaw_abs(x_values: np.ndarray, y_values: np.ndarray, idx: np.ndarray) -> tuple[float, float]:
    indices = np.asarray(idx, dtype=np.int64).reshape(-1)
    ...
    log_y = np.log10(np.maximum(1e-300, np.abs(y_vals)))
...
# 関数: `_fit_fixed_exponent_abs` の入出力契約と処理意図を定義する。

def _fit_fixed_exponent_abs(x_values: np.ndarray, y_values: np.ndarray, idx: np.ndarray, exponent: float) -> float:
    indices = np.asarray(idx, dtype=np.int64).reshape(-1)
    x_vals = np.asarray(x_values, dtype=float).reshape(-1)[indices]
    y_vals = np.asarray(y_values, dtype=float).reshape(-1)[indices]
    denom = np.maximum(1e-300, x_vals**float(exponent))
    return float(np.mean(np.abs(y_vals) / denom))
...
def main() -> None:
    ...
    photon_density_coeff = (2.0 * zeta3 / (math.pi**2)) * ((boltzmann_k / (hbar * speed_of_light)) ** 3)
    photon_flux_coeff = 0.25 * speed_of_light * photon_density_coeff

    temperature_grid_k = np.logspace(0.0, 4.0, 80)
    photon_density = photon_density_coeff * (temperature_grid_k**3)
    photon_flux = photon_flux_coeff * (temperature_grid_k**3)

    ratio_flux_per_density = photon_flux / np.maximum(1e-300, photon_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_flux_per_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_flux_per_density, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_flux_per_density, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_flux_per_density, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        models["powerlaw_abs_fit_2p"] = {
            "params": {"log10_abs_coeff": float(log10_coeff), "exponent": float(exponent), "sign": 1.0},
            "train": m1_train,
            "test": m1_test,
        }

        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_flux_per_density, train_idx, exponent=0.0)
        ratio_pred_fixed = np.abs(coeff_fixed_abs * (temperature_grid_k**0.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_flux_per_density,
            y_pred=ratio_pred_fixed,
            sigma=ratio_sigma,
            param_count=1,
            is_train=True,
        )
        ...
        models["fixed_exponent_0_abs_fit"] = {
            "params": {"abs_coeff": float(coeff_fixed_abs), "exponent": 0.0, "sign": 1.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_photon_flux_per_photon_density_holdout_splits.csv"
    ...
    categories = [str(split_result["name"]) for split_result in split_results]
    y_power = []
    y_fixed = []
    for split_result in split_results:
        models = split_result.get("models") if isinstance(split_result.get("models"), dict) else {}
        model_power = models.get("powerlaw_abs_fit_2p") if isinstance(models, dict) else None
        model_fixed = models.get("fixed_exponent_0_abs_fit") if isinstance(models, dict) else None
    ...
    ax.bar(
        bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 0 abs fit (1p)"
    )
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: photon flux / photon density = c/4")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_flux_per_photon_density_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_flux_per_photon_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "zeta3": zeta3,
                    "photon_density_coeff": photon_density_coeff,
                    "photon_flux_coeff": photon_flux_coeff,
                },
                "assumptions": {
                    "sign_convention": "Phi_n/n is positive for isotropic blackbody radiation; abs-fit with fixed positive sign.",
                ...
                "notes": [
                    "Analytic holdout for photon flux per photon density based on isotropic blackbody identities.",
