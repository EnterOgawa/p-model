#!/usr/bin/env python3
"""Generate 8.7.56.2155-.2158 second post-break registry refresh artifacts."""
...
NEXT_ROUTE = "8.7.56.2159"
FOLLOWUP_ROUTE_NAME = (
    "trial2_numeric_alpha_vector_qball_form_factor_second_post_break_"
    "piecewise_registry_refresh"
)
FOLLOWUP_ROUTE = "8.7.56.2163"
...
def build_formulae() -> dict[str, str]:
    ...
    return {
        "gate_a_rule": "Gate A retains the same second post-break piecewise continuation once it survives farther holdout and monitor windows through harmonic 393216.",
        "gate_b_rule": "Gate B keeps the third post-break piecewise surface closed while the inherited second segment already survives.",
...
def main() -> None:
    ...
    status_hit = find_line(status_text, BRANCH_CLASS) or find_line(status_text, "8.7.56.2155")
    ...
    inventory_ready = bool(
        prior_summary["second_post_break_piecewise_validation_to_393216_supported"]
    )
    gate_a_second_piecewise_validation_to_393216_retained = bool(
        prior_summary["same_second_piecewise_farther_continuation_supported"]
    )
    ...
    rows = [
        sign_base.row(
            ...
            "The registry refresh starts only after the farther audit has already shown that the same second segment survives through harmonic 393216.",
        ),
        sign_base.row(
            "gate_a_second_piecewise_validation_to_393216_retained",
            "pass" if gate_a_second_piecewise_validation_to_393216_retained else "reject",
            "Gate A same second post-break piecewise validation through harmonic 393216 retained",
            sign_base.truth(gate_a_second_piecewise_validation_to_393216_retained),
            "Gate A is retained because the inherited second segment passes both farther holdout and monitor windows.",
        ),
        sign_base.row(
            ...
            "Gate B stays closed because the retry gate blocks a third segment while the inherited second segment still survives.",
        ),
        sign_base.row(
    ...
    summary = {
        ...
        "second_farther_holdout_max_mismatch_abs_error": float(
            prior_summary["second_farther_holdout_max_mismatch_abs_error"]
        ),
        "second_farther_holdout_max_correlation_abs_error": float(
            prior_summary["second_farther_holdout_max_correlation_abs_error"]
        ),
        "second_farther_holdout_max_reconstruction_abs_error": float(
            prior_summary["second_farther_holdout_max_reconstruction_abs_error"]
        ),
        "second_farther_monitor_max_mismatch_abs_error": float(
            prior_summary["second_farther_monitor_max_mismatch_abs_error"]
        ),
        "second_farther_monitor_max_correlation_abs_error": float(
            prior_summary["second_farther_monitor_max_correlation_abs_error"]
        ),
        "second_farther_monitor_max_reconstruction_abs_error": float(
            prior_summary["second_farther_monitor_max_reconstruction_abs_error"]
        ),
        "same_second_piecewise_farther_continuation_supported": gate_a_second_piecewise_validation_to_393216_retained,
        "second_post_break_piecewise_validation_to_393216_supported": inventory_ready,
        "third_post_break_piecewise_surface_admissible_now": gate_b_third_piecewise_reactivation_selected,
        "gate_a_second_piecewise_validation_to_393216_retained": gate_a_second_piecewise_validation_to_393216_retained,
    ...
    declaration_payload = sign_base.payload(
        ...
        },
        {
            ...
            "hits": {
                ...
                "roadmap_branch_hit": find_line(roadmap_text, "8.7.56.2155-.2158"),
    ...
    route_sync_rows = [
        sign_base.row(
            ...
            "The registry refresh is only honest if the official status still points to the same second post-break farther branch before the new state is frozen.",
        ),
        sign_base.row(
            "roadmap_synced",
            "pass" if bool(find_line(roadmap_text, "8.7.56.2155-.2158")) else "reject",
            "ROADMAP sync target present",
            sign_base.truth(bool(find_line(roadmap_text, "8.7.56.2155-.2158"))),
            "The public roadmap must expose the same second post-break registry branch before route sync can proceed.",
        ),
        sign_base.row(
            "long_horizon_synced",
            "pass" if bool(find_line(long_text, ".2147-.2150")) else "reject",
