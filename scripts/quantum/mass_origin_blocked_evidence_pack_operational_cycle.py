...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_evidence_pack_operational_cycle.py

Step 8.7.55.2.24:
Freeze one current-state operational-cycle artifact for the mass-origin
blocked evidence pack, so downstream readers can see whether the continuity
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_evidence_pack_operational_cycle_rows.csv
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_evidence_pack_operational_cycle.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_evidence_pack_operational_cycle_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.24"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the operational-cycle state for the mass-origin blocked evidence pack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _build_rows(cycle_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "metric": "blocked evidence-pack continuity state is unchanged",
        ...
        },
        {
            ...
            "status": "pass" if cycle_state.get("rerun_trigger") == "blocked_evidence_pack_continuity_state_changed" else "reject",
            "metric": "rerun trigger matches the continuity trigger contract",
            "value": 1.0 if cycle_state.get("rerun_trigger") == "blocked_evidence_pack_continuity_state_changed" else 0.0,
            "note": f"Current rerun trigger = {cycle_state.get('rerun_trigger')}.",
        },
        {
            "row_id": "apply_chain_step_count_expected",
            "status": "pass" if int(cycle_state.get("apply_chain_step_count", 0)) == 6 else "reject",
            "metric": "rerun/apply chain keeps the expected six-step order",
            "value": float(cycle_state.get("apply_chain_step_count", 0)),
            "note": "The operational cycle expects six steps: latent inventory, same-sector gate, blocked-state reopen, hold monitor settle, manifest refresh, continuity settle.",
        },
        {
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    if has_reject:
        ...
        next_action = "repair_trigger_or_apply_chain_contract"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(cycle_state.get(\"keep_mass_origin_branch_blocked\", True))` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_operational_cycle"
    else:
        overall_status = "pass"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked evidence-pack operational cycle",
        },
        "inputs": {
            "blocked_evidence_pack_continuity_metrics_json": _rel(CONTINUITY_JSON),
            "blocked_evidence_pack_rerun_policy_json": _rel(RERUN_POLICY_JSON),
        },
        "intent": "Freeze one current-state operational-cycle decision for the mass-origin blocked evidence pack without executing the rerun chain.",
        "summary": {
