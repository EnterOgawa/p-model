"""
目的: 黒体 holdout 監査の photon density per photon flux 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_density_per_flux = photon_density / np.maximum(1e-300, photon_flux)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_density_per_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_density_per_flux, train_idx)
        ratio_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_density_per_flux,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_density_per_flux, train_idx, exponent=0.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k**0.0)
        ...
        models["fixed_exponent_0_fit"] = {
            "params": {"coeff": float(coeff_fixed), "exponent": 0.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_photon_density_per_photon_flux_holdout_splits.csv"
    ...
    for split_result in split_results:
        ...
        model_fixed = models.get("fixed_exponent_0_fit") if isinstance(models, dict) else None
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 0 fit (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: photon density / photon flux = 4/c")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_density_per_photon_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_density_per_photon_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "n/Phi_n is positive for isotropic blackbody radiation.",
                ...
                "notes": [
                    "Analytic holdout for photon density per photon flux based on isotropic blackbody identities.",
