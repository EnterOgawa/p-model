...
def main() -> None:
    ...
    ratio_enthalpy_per_photon_flux = enthalpy_density / np.maximum(1e-300, photon_flux)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_enthalpy_per_photon_flux))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_enthalpy_per_photon_flux, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_enthalpy_per_photon_flux,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(
            temperature_grid_k,
            ratio_enthalpy_per_photon_flux,
            train_idx,
            exponent=1.0,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_enthalpy_per_photon_flux_holdout_splits.csv"
