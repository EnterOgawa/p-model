...
# -*- coding: utf-8 -*-
"""
mass_origin_source_wording_blocker_split_refresh.py

Step 8.7.55.2.114:
Reinject the source-wording split-route results into the blocked second-route
...
Outputs:
  ...
  - output/public/quantum/mass_origin_source_wording_blocker_split_refresh_rows.csv
"""
...
PRIOR_REFRESH_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_source_wording_branch_refresh_metrics.json"
...
OUT_JSON = ROOT / "output" / "public" / "quantum" / "mass_origin_source_wording_blocker_split_refresh_metrics.json"
OUT_CSV = ROOT / "output" / "public" / "quantum" / "mass_origin_source_wording_blocker_split_refresh_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.114"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Refresh blocked second-route handoff eligibility after source-wording blocker split retries.",
    )
    parser.add_argument(
        "--step-tag",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    remaining_source_level_blockers = _ordered_unique(
        [
            str(shell_anchor_retry_summary.get("shell_anchor_wording_nonclosure_reason_or_none") or ""),
            str(explicit_mapping_retry_summary.get("explicit_mapping_wording_nonclosure_reason_or_none") or ""),
        ]
    )

    rows = [
        {
            "row_id": "source_wording_blocker_split_refresh_complete",
            "status": "pass",
            "metric": "source-wording blocker split refresh complete",
            "value": 1.0,
            "note": "This refresh reinjects the source-wording split-route retries into the blocked second-route stack and refreezes .83-.84 handoff eligibility.",
        },
        {
            "row_id": "source_wording_blocker_split_same_sector_tiebreak_target_value",
            "status": "pass" if same_sector_tiebreak_target_value_available else "watch",
            "metric": "same-sector tie-break target value available after source-wording split refresh",
            "value": 1.0 if same_sector_tiebreak_target_value_available else 0.0,
            "note": (
                f"Target source kind is {target_source_kind_or_none}."
                if same_sector_tiebreak_target_value_available
                else f"Source-wording split refresh remains blocked by {remaining_source_level_blockers}."
            ),
        },
        {
            "row_id": "source_wording_blocker_split_single_public_vpp_shape",
            "status": "pass" if single_public_vpp_shape_available else "reject",
            "metric": "single public V(|P|) shape available after source-wording split refresh",
        ...
        {
            "row_id": "source_wording_blocker_split_positive_same_sector_public_artifact",
            "status": "pass" if positive_particle_sector_chi_p_to_vpp_public_artifact_available else "reject",
            "metric": "positive particle-sector chi_P -> V''(|P|_*) public artifact available after source-wording split refresh",
            ...
            "note": (
                ...
                else "Promotion remains absent because explicit mapping wording closure and single-shape closure did not reopen together."
            ),
        },
        {
            "row_id": "solver_ready_row_promoted_to_pass",
            "status": "pass" if solver_ready_row_promoted_to_pass else "reject",
            "metric": "solver-ready row promoted to pass after source-wording split refresh",
            "value": 1.0 if solver_ready_row_promoted_to_pass else 0.0,
            "note": (
                "All reopen prerequisites now close simultaneously after source-wording split reinjection."
        ...
        },
        {
            ...
            "metric": "mass-origin branch reopen ready after source-wording split refresh",
        ...
        },
        {
            ...
            "metric": "handoff to 8.7.55.2.83-.84 allowed after source-wording split refresh",
            ...
            "note": (
                ...
                else "Handoff remains blocked because source-wording split reinjection did not reopen the branch."
            ),
        },
        {
            "row_id": "source_wording_blocker_split_refresh_source_level_blocker_count",
            "status": "inventory",
            "metric": "remaining source-level blocker count after source-wording split refresh",
            ...
            "note": (
                ...
                else "No source-level blockers remain after source-wording split refresh."
            ),
        },
        {
            "row_id": "source_wording_blocker_split_refresh_remaining_artifact_count",
            "status": "inventory",
            "metric": "remaining artifact-level missing count after source-wording split refresh",
            ...
            "note": (
                ...
                else "No artifact-level blockers remain after source-wording split refresh."
            ),
        },
    ]

    overall_status = (
        "source_wording_blocker_split_refresh_reopen_ready"
        if hand_off_to_8_7_55_2_83
        else "source_wording_blocker_split_refresh_still_blocked"
    )
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "source-wording blocker split refresh / handoff",
        },
        "inputs": {
            "mass_origin_source_wording_branch_refresh_json": _relative_str(PRIOR_REFRESH_JSON),
        ...
        "intent": "Refresh the blocked second-route stack after source-wording blocker split retries and refreeze whether the branch can hand off to .83-.84.",
        "formulas": {
            "target_value_rule": "same_sector_tiebreak_target_value_available iff the shell-anchor wording retry now closes an admissible wording route",
            "single_shape_rule": "single_public_vpp_shape_available remains tied to the prior refreshed candidate selection unless the new target-driven route reopens it",
