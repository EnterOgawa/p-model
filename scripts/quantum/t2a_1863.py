#!/usr/bin/env python3
"""Generate 8.7.56.1863-.1866 post-dormant wait-restore artifacts."""
...
POST_DORMANT_AUDIT_GATE = (
    PUBLIC_OUT
    / "q_8_7_56_1859_1862_post_dormant_reactivation_audit_declaration_gate_metrics.json"
)

STEP_TAG = "8.7.56.1863-1866"
STEP_NAME = (
    "Trial-2 numeric alpha vector Q-ball form-factor post-dormant wait "
    "restore / registry refresh"
)
STEM = build_compact_artifact_stem(
    STEP_TAG,
    "post_dormant_wait_restore_registry_refresh",
    prefix="q",
)
...
def main() -> None:
    ...
    audit_summary = read_json(POST_DORMANT_AUDIT_GATE)["summary"]
    ...
    post_dormant_registry_refreshed = True
    ...
    rows = [
        row("inventory_ready", "pass" if inventory_ready else "reject", "post-dormant registry-refresh inventory ready", truth(inventory_ready), "Registry refresh starts only after the post-dormant audit has declared no new trigger."),
        ...
        row("post_dormant_registry_refreshed", "pass", "post-dormant registry refreshed", truth(post_dormant_registry_refreshed), "The closed family remains frozen and its dormant registry is now refreshed rather than reopened."),
        row("same_level_post_dormant_retry_admissible", "reject", "same-level post-dormant retry admissible", truth(same_level_post_dormant_retry_admissible), "Same-level retry stays blocked after the dormant registry refresh."),
    ...
    summary = {
        ...
        "post_dormant_registry_refreshed": post_dormant_registry_refreshed,
    ...
    decision = {
        "overall_status": "vector_qball_form_factor_post_dormant_wait_restore_declared",
    ...
    evidence = {
        "formulas": formulas,
        "hits": {
            "status_branch_hit": hit(status_text, "8.7.56.1863"),
            "roadmap_branch_hit": hit(roadmap_text, "8.7.56.1863-.1866"),
            ...
            "unified_roadmap_hit": hit(unified_text, "93. `.1863-.1866`"),
            "long_roadmap_hit": hit(long_text, "8.7.56.1863-.1866"),
    ...
    declaration_payload = payload(
        ...
        STEP_NAME + " declaration gate",
        {
            "source_files": {
                ...
                "post_dormant_audit_gate": display_path(POST_DORMANT_AUDIT_GATE),
            },
            "constants": {
    ...
    route_payload = payload(
        ...
        [
            row("dormant_registry_refreshed", "pass", "dormant registry refreshed", 1.0, "The post-dormant family remains frozen and only future substantive reactivation is honest."),
            row("next_route_fixed", "pass", "next route fixed", 1.0, "The next official branch is the second post-dormant conditional reactivation audit."),
        ],
        {
            "trial2_numeric_alpha_problem_classification": BRANCH_CLASS,
            "selected_next_generation_route": NEXT_ROUTE_NAME,
        ...
        {
            "overall_status": "vector_qball_form_factor_post_dormant_wait_route_synced",
            "branch_completed": True,
            "next_required_artifacts": [NEXT_ROUTE_NAME],
        },
        {"formulas": formulas},
    ...
    print("[ok] 8.7.56.1863-.1866 post-dormant wait-restore artifacts generated")
