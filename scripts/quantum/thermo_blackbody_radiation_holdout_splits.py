"""
目的: 黒体 holdout 監査の radiation 指標図を再生成する。
...
def main() -> None:
    ...
    energy_sigma = np.maximum(1e-12, 1e-6 * energy_density)  # 1 ppm relative tolerance
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, energy_density, train_idx)
        energy_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=energy_density, y_pred=energy_pred, sigma=energy_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=energy_density, y_pred=energy_pred, sigma=energy_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, energy_density, train_idx, exponent=4.0)
        energy_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=energy_density,
            y_pred=energy_pred_fixed,
            sigma=energy_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_radiation_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody radiation holdout: u(T) scaling across temperature bands")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_radiation_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_radiation_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody radiation u(T)=a T^4 using SI constants.",
