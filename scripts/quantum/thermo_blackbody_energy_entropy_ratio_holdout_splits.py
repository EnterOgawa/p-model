"""
目的: 黒体 holdout 監査の energy entropy ratio 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_energy_entropy = energy_density / np.maximum(1e-300, entropy_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_energy_entropy)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_energy_entropy, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_energy_entropy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_energy_entropy, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_energy_entropy, train_idx, exponent=1.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k**1.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_energy_entropy,
    ...
    out_csv = out_dir / "thermo_blackbody_energy_entropy_ratio_holdout_splits.csv"
    ...
    for split_result in split_results:
        ...
        model_fixed = models.get("fixed_exponent_1_fit") if isinstance(models, dict) else None
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: u/s = (3/4) T")
    ax.set_title("Blackbody ratio holdout: u/s = (3/4) T")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_energy_entropy_ratio_holdout_splits.png"
