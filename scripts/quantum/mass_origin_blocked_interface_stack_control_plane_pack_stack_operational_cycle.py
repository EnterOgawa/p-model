...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_pack_stack_operational_cycle.py

Step 8.7.55.2.44:
Freeze one current-state operational-cycle artifact for the mass-origin
blocked interface-stack control-plane pack stack, so downstream readers can
tell whether the control-plane pack stack continuity artifact and the
control-plane pack stack rerun policy still agree on the present
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_operational_cycle_rows.csv
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_stack_operational_cycle.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_stack_operational_cycle_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.44"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the operational-cycle state for the mass-origin blocked interface-stack control-plane pack stack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _extract_cycle_state(continuity: Dict[str, Any], rerun_policy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "control_plane_pack_stack_complete": bool(rerun_decision.get("control_plane_pack_stack_complete", False)),
...
def _build_rows(cycle_state: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "metric": "control-plane pack stack continuity state is unchanged",
            "value": 1.0 if not bool(cycle_state.get("continuity_state_changed", False)) else 0.0,
            "note": "The rerun chain should stay idle until the control-plane pack stack continuity state changes.",
        },
        {
            "row_id": "rerun_required_now_false",
            "status": "pass" if not bool(cycle_state.get("rerun_required_now", False)) else "watch",
            "metric": "control-plane pack stack rerun policy does not currently require the chain to run",
            "value": 1.0 if not bool(cycle_state.get("rerun_required_now", False)) else 0.0,
            "note": "This row is watch only when the pack-stack policy already says the chain must run now.",
        },
        {
            "row_id": "rerun_trigger_is_control_plane_pack_stack_continuity_state_changed",
            "status": (
                "pass"
                if cycle_state.get("rerun_trigger") == "blocked_interface_stack_control_plane_pack_stack_continuity_state_changed"
                else "reject"
            ),
            "metric": "rerun trigger matches the control-plane pack stack continuity trigger contract",
            "value": (
                1.0
                if cycle_state.get("rerun_trigger") == "blocked_interface_stack_control_plane_pack_stack_continuity_state_changed"
                else 0.0
        ...
        },
        {
            ...
            "note": "The control-plane pack stack operational cycle expects three steps: pack-stack audit, pack-stack manifest, pack-stack continuity settle.",
        },
        {
            "row_id": "apply_chain_signature_present",
            "status": "pass" if apply_chain_signature else "reject",
            "metric": "control-plane pack stack rerun/apply chain signature is present",
            "value": 1.0 if apply_chain_signature else 0.0,
            "note": "The chain signature lets downstream readers detect pack-stack policy drift.",
        },
        {
        ...
        {
            "row_id": "control_plane_pack_stack_complete",
            "status": "pass" if bool(cycle_state.get("control_plane_pack_stack_complete", False)) else "reject",
            "metric": "control-plane pack stack remains complete",
            "value": 1.0 if bool(cycle_state.get("control_plane_pack_stack_complete", False)) else 0.0,
            "note": "The operational cycle assumes the full blocked control-plane pack stack remains complete.",
        },
        {
        ...
        },
        {
            ...
            "metric": "mass-origin branch remains blocked in the control-plane pack stack operational cycle",
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    if has_reject:
        ...
        next_action = "repair_control_plane_pack_stack_trigger_or_apply_chain_contract"
    ...
    # 条件分岐: 前段条件が不成立で、`bool(cycle_state.get("keep_mass_origin_branch_blocked", True))` を追加評価する。
        ...
        next_action = "wait_for_new_public_artifact_then_rerun_control_plane_pack_stack_operational_cycle"
    else:
        overall_status = "pass"
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane pack stack operational cycle",
        },
        "inputs": {
        ...
        "intent": "Freeze one current-state operational-cycle decision for the mass-origin blocked interface-stack control-plane pack stack without executing the rerun chain.",
        "summary": {
            ...
            "control_plane_pack_stack_complete": bool(cycle_state.get("control_plane_pack_stack_complete", False)),
            "next_required_artifacts": cycle_state.get("next_required_artifacts", []),
        },
        "checks": rows,
        "decision": {
