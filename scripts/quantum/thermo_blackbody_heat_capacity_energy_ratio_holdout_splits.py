"""
目的: 黒体 holdout 監査の heat capacity energy ratio 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_heat_capacity_energy = heat_capacity_density / np.maximum(1e-300, energy_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_heat_capacity_energy))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_heat_capacity_energy, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_heat_capacity_energy,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_heat_capacity_energy, train_idx, exponent=-1.0)
    ...
    out_csv = out_dir / "thermo_blackbody_heat_capacity_energy_ratio_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: cV/u \u221d T^-1")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_heat_capacity_energy_ratio_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_heat_capacity_energy_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "cV/u is positive for photon gas; model fit is applied to abs(cV/u) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for cV/u (heat-capacity density per energy density) based on blackbody identities.",
