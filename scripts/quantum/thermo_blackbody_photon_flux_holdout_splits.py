"""
目的: 黒体 holdout 監査の photon flux 指標図を再生成する。
...
def main() -> None:
    ...
    flux_sigma = np.maximum(1e-12, 1e-6 * photon_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, photon_flux, train_idx)
        flux_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=photon_flux, y_pred=flux_pred, sigma=flux_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=photon_flux, y_pred=flux_pred, sigma=flux_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, photon_flux, train_idx, exponent=3.0)
        flux_pred_fixed = coeff_fixed * (temperature_grid_k**3.0)
    ...
    out_csv = out_dir / "thermo_blackbody_photon_flux_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody photon flux holdout: Phi_n(T)=(c/4) n(T)")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_flux_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody photon flux Phi_n(T)=(c/4)*n(T).",
