"""
目的: 黒体 holdout 監査の peak radiance 指標図を再生成する。
...
def _wien_peak_x_wavelength() -> float:
    # Solve 5(1 - e^{-x}) = x for the wavelength peak of B_λ.
    lo = 4.0
    hi = 6.0
    for _ in range(80):
        mid = 0.5 * (lo + hi)
...
def main() -> None:
    ...
    x_peak = _wien_peak_x_wavelength()
    wien_b = (planck_h * speed_of_light) / (boltzmann_k * x_peak)
    denom = math.expm1(x_peak)
    peak_coeff = (2.0 * planck_h * speed_of_light**2) / (wien_b**5) / denom

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    peak_radiance = peak_coeff * (temperature_grid_k**5)

    radiance_sigma = np.maximum(1e-12, 1e-6 * peak_radiance)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, peak_radiance, train_idx)
        radiance_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=peak_radiance, y_pred=radiance_pred, sigma=radiance_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=peak_radiance, y_pred=radiance_pred, sigma=radiance_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, peak_radiance, train_idx, exponent=5.0)
        radiance_pred_fixed = coeff_fixed * (temperature_grid_k**5.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=peak_radiance,
            y_pred=radiance_pred_fixed,
            sigma=radiance_sigma,
            param_count=1,
            is_train=True,
        )
        ...
        models["fixed_exponent_5_fit"] = {
            "params": {"coeff": float(coeff_fixed), "exponent": 5.0},
            "train": m2_train,
            "test": m2_test,
        }
    ...
    out_csv = out_dir / "thermo_blackbody_peak_radiance_holdout_splits.csv"
    ...
    for sp in split_results:
        ...
        model_fixed = models.get("fixed_exponent_5_fit") if isinstance(models, dict) else None
    ...
    ax.bar(bar_positions + bar_width / 2, y_fixed, width=bar_width, color="#ff7f0e", alpha=0.85, label="fixed exponent 5 (1p)")
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody peak spectral radiance holdout: B_λ,max ∝ T^5")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_peak_radiance_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_peak_radiance_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody peak spectral radiance at the Planck B_λ maximum.",
                    "Uses λ_max=b/T and B_λ,max=(2hc^2/λ_max^5)/(exp(x)-1) leading to B_λ,max ∝ T^5.",
                ],
            },
            ensure_ascii=False,
            indent=2,
