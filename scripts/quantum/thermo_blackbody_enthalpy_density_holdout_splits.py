"""
目的: 黒体 holdout 監査の enthalpy density 指標図を再生成する。
...
def main() -> None:
    ...
    enthalpy_coeff = (4.0 / 3.0) * radiation_constant_a

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    enthalpy_density = enthalpy_coeff * (temperature_grid_k**4)

    enthalpy_sigma = np.maximum(1e-12, 1e-6 * enthalpy_density)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, enthalpy_density, train_idx)
        enthalpy_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=enthalpy_density, y_pred=enthalpy_pred, sigma=enthalpy_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=enthalpy_density, y_pred=enthalpy_pred, sigma=enthalpy_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, enthalpy_density, train_idx, exponent=4.0)
        enthalpy_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=enthalpy_density,
            y_pred=enthalpy_pred_fixed,
            sigma=enthalpy_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_enthalpy_density_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody enthalpy density holdout: h = (4/3) a T^4")
    ax.set_title("Blackbody enthalpy density holdout: h = (4/3) a T^4")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_enthalpy_density_holdout_splits.png"
