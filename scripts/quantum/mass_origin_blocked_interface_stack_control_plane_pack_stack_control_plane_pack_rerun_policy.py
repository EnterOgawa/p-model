...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy.py

Step 8.7.55.2.53:
Freeze the rerun/apply policy for the mass-origin blocked interface-stack
control-plane pack stack control-plane pack, so downstream operators know
which top-level public canonical scripts must be rerun, and in what order,
when the control-plane pack continuity state changes.
...
Outputs:
  ...
  - output/public/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy_rows.csv

Assumptions:
  - The control-plane pack continuity audit is the sole trigger source for
    deciding whether the full top-level control-plane pack reader chain should
    be rerun.
...
CONTINUITY_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_continuity_metrics.json"
...
OUT_JSON = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy.json"
OUT_CSV = PUBLIC_QUANTUM_DIR / "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy_rows.csv"
DEFAULT_STEP_TAG = "8.7.55.2.53"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Freeze the rerun/apply policy for the mass-origin blocked interface-stack control-plane pack stack control-plane pack.",
    )
    parser.add_argument(
        "--step-tag",
...
def _build_apply_chain_rows(step_tag: str, rerun_required_now: bool) -> List[Dict[str, Any]]:
    chain_specs = [
        {
            "command_id": "refresh_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_manifest",
            "script": "scripts/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_manifest.py",
            "expected_output": "output/public/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_manifest.json",
            "run_when": "control_plane_pack_stack_control_plane_pack_continuity_state_changed",
            "settle_required": False,
            "note": "Re-freeze the top-level control-plane pack manifest after upstream inputs change.",
        },
        {
            "command_id": "settle_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_continuity",
            "script": "scripts/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_continuity_audit.py",
            "expected_output": "output/public/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_continuity_metrics.json",
            "run_when": "after_control_plane_pack_stack_control_plane_pack_manifest_refresh",
            "settle_required": True,
            "note": "Rerun until the top-level control-plane pack continuity artifact returns no_change_hold against the refreshed control-plane pack manifest.",
        },
        {
            "command_id": "refresh_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy",
            "script": "scripts/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy.py",
            "expected_output": "output/public/quantum/mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_rerun_policy.json",
            "run_when": "after_control_plane_pack_stack_control_plane_pack_continuity_settled",
            "settle_required": False,
            "note": "Refresh the current rerun-policy artifact after the continuity layer has settled.",
        },
    ]
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    # 条件分岐: 前段条件が不成立で、`continuity_state.get("continuity_decision") == "no_change_hold"` を追加評価する。
        ...
        overall_status = "watch"
        next_action = "wait_for_new_public_artifact_then_rerun_control_plane_pack_stack_control_plane_pack_policy_chain"
    else:
        decision_text = "policy_locked_watch_state"
        overall_status = "watch"
        next_action = str(
            continuity_state.get("watchpack_next_action")
            or "keep_control_plane_pack_stack_control_plane_pack_policy_active_until_continuity_changes"
        )
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane pack stack control-plane pack rerun policy",
        },
        "inputs": {
        ...
        "intent": "Freeze the rerun/apply chain that must be used when the mass-origin blocked interface-stack control-plane pack continuity state changes.",
        "summary": {
        ...
        "decision": {
            ...
            "rerun_required_now": rerun_required_now,
            "rerun_trigger": "blocked_interface_stack_control_plane_pack_stack_control_plane_pack_continuity_state_changed",
            "control_plane_pack_stack_control_plane_pack_complete": bool(
