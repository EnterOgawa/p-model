...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch Delft (Hensen et al. 2016; Sci Rep 6, 30289) second loophole-free Bell test open data (trial table)."
        )
    )
    parser.add_argument(
        "--offline",
    ...
    src_dir = root / "data" / "quantum" / "sources" / "delft_hensen2016_srep30289"
    src_dir.mkdir(parents=True, exist_ok=True)

    # 4TU.ResearchData (public). Landing page:
    # https://data.4tu.nl/articles/dataset/Second_loophole-free_Bell_test_with_electron_spins_separated_by_1_3_km/12694403
    #
    # DOI: 10.4121/uuid:53644d31-d862-4f9f-9ad2-0b571874b829
    # File: data.zip (131,394 bytes)
    files: list[FileSpec] = [
        FileSpec(
            url="https://data.4tu.nl/file/86781ed5-3d14-4ac1-89a9-5e5cddecd748/3ef090cd-4426-48e7-8e77-44d545491667",
            relpath="data.zip",
            expected_bytes=131_394,
        )
    ]
    ...
    manifest = {
        ...
        "dataset": "Delft second loophole-free Bell test (Hensen et al. 2016; Sci Rep 6, 30289) open data (trial table)",
        "doi_dataset": "10.4121/uuid:53644d31-d862-4f9f-9ad2-0b571874b829",
        "landing_page": "https://data.4tu.nl/articles/dataset/Second_loophole-free_Bell_test_with_electron_spins_separated_by_1_3_km/12694403",
        "files": [],
    }
    for spec in files:
        path = src_dir / spec.relpath
