"""
目的: 黒体 holdout 監査の flux per photon 指標図を再生成する。
...
def main() -> None:
    ...
    ratio_flux_per_photon = energy_flux / np.maximum(1e-300, photon_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_flux_per_photon))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_flux_per_photon, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_flux_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_flux_per_photon, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_flux_per_photon, train_idx, exponent=1.0)
        ratio_pred_fixed = np.abs(coeff_fixed_abs * (temperature_grid_k**1.0))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_flux_per_photon,
    ...
    out_csv = out_dir / "thermo_blackbody_flux_per_photon_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: M/n ∝ T")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_flux_per_photon_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_flux_per_photon_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "M/n is positive for photon gas; model fit is applied to abs(M/n) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for M/n (energy-flux per photon number density) based on blackbody identities.",
