...
def main() -> None:
    ...
    enthalpy_flux_coeff = (4.0 / 3.0) * stefan_boltzmann_sigma

    temperature_grid_k = np.logspace(0.0, 4.0, 80)
    enthalpy_flux = enthalpy_flux_coeff * (temperature_grid_k**4)
    enthalpy_flux_sigma = np.maximum(1e-12, 1e-6 * enthalpy_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, enthalpy_flux, train_idx)
        flux_pred = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=enthalpy_flux, y_pred=flux_pred, sigma=enthalpy_flux_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=enthalpy_flux, y_pred=flux_pred, sigma=enthalpy_flux_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, enthalpy_flux, train_idx, exponent=4.0)
        flux_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=enthalpy_flux,
            y_pred=flux_pred_fixed,
            sigma=enthalpy_flux_sigma,
            param_count=1,
            is_train=True,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_enthalpy_flux_holdout_splits.csv"
    ...
    ax.set_title("Blackbody enthalpy flux holdout: J_h(T)=(c/4)h(T)")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_enthalpy_flux_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_enthalpy_flux_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "enthalpy_flux_coeff": enthalpy_flux_coeff,
                },
                "assumptions": {
                ...
                "notes": [
                    "Analytic holdout for blackbody enthalpy flux J_h(T)=(c/4)h(T)=(4/3)sigma*T^4.",
