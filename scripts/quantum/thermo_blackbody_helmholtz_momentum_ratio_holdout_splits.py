"""
目的: 黒体 holdout 監査の helmholtz momentum ratio 指標図を再生成する。
...
def main() -> None:
    ...
    free_energy_density = -(radiation_constant_a / 3.0) * (temperature_grid_k**4)
    momentum_density = (radiation_constant_a / speed_of_light) * (temperature_grid_k**4)

    ratio_helmholtz_momentum = free_energy_density / np.maximum(1e-300, momentum_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * np.abs(ratio_helmholtz_momentum))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_helmholtz_momentum, train_idx)
        ratio_pred_abs = (10**log10_coeff) * (temperature_grid_k**exponent)
        ratio_pred = -np.abs(ratio_pred_abs)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_helmholtz_momentum, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_helmholtz_momentum, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_helmholtz_momentum, train_idx, exponent=0.0)
        ratio_pred_fixed = -np.abs(coeff_fixed_abs * np.ones_like(temperature_grid_k))
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_helmholtz_momentum,
    ...
    out_csv = out_dir / "thermo_blackbody_helmholtz_momentum_ratio_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: f/g = -c/3")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_helmholtz_momentum_ratio_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_helmholtz_momentum_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "f/g is negative for photon gas; model fit is applied to abs(f/g) and sign=-1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for f/g = -c/3 based on blackbody Helmholtz free-energy density and momentum-density identities.",
