"""
目的: 黒体 holdout 監査の photon flux per momentum density 指標図を再生成する。
...
def main() -> None:
    ...
    momentum_density = energy_density / speed_of_light
    photon_density = photon_density_coeff * (temperature_grid_k**3)
    photon_flux = (speed_of_light / 4.0) * photon_density

    ratio_photon_flux_per_momentum_density = photon_flux / np.maximum(1e-300, momentum_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_photon_flux_per_momentum_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_photon_flux_per_momentum_density, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_photon_flux_per_momentum_density,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(temperature_grid_k, ratio_photon_flux_per_momentum_density, train_idx, exponent=-1.0)
    ...
    out_csv = out_dir / "thermo_blackbody_photon_flux_per_momentum_density_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: Φ_n/g ∝ T^-1")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_flux_per_momentum_density_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_flux_per_momentum_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "Φ_n/g is positive for photon gas; model fit is applied to abs(Φ_n/g) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for Φ_n/g (photon flux per momentum density) based on blackbody identities.",
