...
def main() -> None:
    ...
    nu_lambda_product = lambda_peak_m * nu_peak_hz

    product_sigma = np.maximum(1e-30, 1e-6 * np.abs(nu_lambda_product))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, nu_lambda_product, train_idx)
        pred_powerlaw = (10**log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=nu_lambda_product, y_pred=pred_powerlaw, sigma=product_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=nu_lambda_product, y_pred=pred_powerlaw, sigma=product_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, nu_lambda_product, train_idx, exponent=0.0)
        pred_fixed = coeff_fixed * np.ones_like(temperature_grid_k)
        m2_train = _metrics_for_idx(
            idx=train_idx, y_obs=nu_lambda_product, y_pred=pred_fixed, sigma=product_sigma, param_count=1, is_train=True
        )
        m2_test = _metrics_for_idx(
            idx=test_idx, y_obs=nu_lambda_product, y_pred=pred_fixed, sigma=product_sigma, param_count=1, is_train=False
        )
    ...
    out_csv = out_dir / "thermo_blackbody_peak_frequency_wavelength_product_holdout_splits.csv"
    ...
    out_metrics = out_dir / "thermo_blackbody_peak_frequency_wavelength_product_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "product_reference_m_per_s": product_reference,
                },
                "assumptions": {
