...
# -*- coding: utf-8 -*-
"""
mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_stack_manifest.py

Step 8.7.55.2.66:
Freeze a single public manifest for the current blocked interface-stack
control-plane pack stack control-plane pack stack stack stack of the
mass-origin branch, so downstream readers can inspect the complete
top-level control-plane pack stack stack stack without reopening each JSON
artifact manually.
...
Assumptions:
  - The control-plane pack stack stack stack is complete only if every
    current top-level control-plane pack stack stack artifact and the
    terminal control-plane pack stack stack stack audit exist together.
...
DEFAULT_STEP_TAG = "8.7.55.2.66"

ARTIFACT_SPECS = [
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_manifest",
        "role": "control_plane_pack_stack_control_plane_pack_stack_stack_stack_input",
        "path": CONTROL_PLANE_PACK_STACK_STACK_MANIFEST_JSON,
        "note": "Frozen manifest for the current top-level control-plane pack stack stack layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_continuity",
        "role": "control_plane_pack_stack_control_plane_pack_stack_stack_stack_input",
        "path": CONTROL_PLANE_PACK_STACK_STACK_CONTINUITY_JSON,
        "note": "Continuity audit for the current top-level control-plane pack stack stack layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_rerun_policy",
        "role": "control_plane_pack_stack_control_plane_pack_stack_stack_stack_input",
        "path": CONTROL_PLANE_PACK_STACK_STACK_RERUN_POLICY_JSON,
        "note": "Rerun/apply policy for the current top-level control-plane pack stack stack layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_operational_cycle",
        "role": "control_plane_pack_stack_control_plane_pack_stack_stack_stack_input",
        "path": CONTROL_PLANE_PACK_STACK_STACK_OPERATIONAL_CYCLE_JSON,
        "note": "Operational-cycle decision for the current top-level control-plane pack stack stack layer.",
    },
    {
        "artifact_id": "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_stack_audit",
        "role": "control_plane_pack_stack_control_plane_pack_stack_stack_stack_terminal",
        "path": CONTROL_PLANE_PACK_STACK_STACK_STACK_AUDIT_JSON,
        "note": "Top-level control-plane pack stack stack stack audit across the current control-plane pack stack stack stack.",
    },
]
        ...
        rows.append(
            {
                ...
                "status": "pass",
                "metric": f"{spec['role']} artifact present in control-plane pack stack stack stack manifest",
                "value": 1.0,
...
def _build_payload(step_tag: str) -> Dict[str, Any]:
    ...
    terminal_row = next(
        row
        for row in rows
        if row["row_id"] == "mass_origin_blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_stack_audit"
    )
    terminal_payload = _read_json(CONTROL_PLANE_PACK_STACK_STACK_STACK_AUDIT_JSON)
    ...
    continuity_payload = _read_json(CONTROL_PLANE_PACK_STACK_STACK_CONTINUITY_JSON)
    ...
    return {
        ...
        "phase": {
            "phase": 8,
            "step": step_tag,
            "name": "mass-origin blocked interface-stack control-plane pack stack control-plane pack stack stack stack manifest",
        },
        "inputs": {
            "public_quantum_dir": _rel(PUBLIC_QUANTUM_DIR),
            "artifact_count": len(rows),
        },
        "intent": "Freeze the current blocked interface-stack control-plane pack stack control-plane pack stack stack stack of the mass-origin branch as a single public canonical manifest.",
        "rows": rows,
        "summary": {
            "artifact_count": len(rows),
            ...
            "pack_stack_stack_stack_audit_status": str(terminal_row.get("artifact_status", "")),
            "next_required_artifacts": next_required_artifacts,
        },
        "decision": {
            "overall_status": "blocked_interface_stack_control_plane_pack_stack_control_plane_pack_stack_stack_stack_manifest_frozen",
            ...
            "next_required_artifacts": next_required_artifacts,
            "next_action": "wait_for_new_public_artifact_then_refresh_control_plane_pack_stack_control_plane_pack_stack_stack_stack_manifest",
        },
        "evidence": {
            "terminal_control_plane_pack_stack_control_plane_pack_stack_stack_stack_audit": {
                "path": _rel(CONTROL_PLANE_PACK_STACK_STACK_STACK_AUDIT_JSON),
                "phase_step": _extract_phase_step(terminal_payload),
                "artifact_status": str(terminal_row.get("artifact_status", "")),
            },
            "continuity_artifact": {
                "path": _rel(CONTROL_PLANE_PACK_STACK_STACK_CONTINUITY_JSON),
