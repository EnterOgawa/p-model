...
def main() -> None:
    ...
    ratio_entropy_per_momentum_density = entropy_density / np.maximum(1e-300, momentum_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_entropy_per_momentum_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_entropy_per_momentum_density, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_entropy_per_momentum_density,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(
            temperature_grid_k,
            ratio_entropy_per_momentum_density,
            train_idx,
            exponent=-1.0,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_entropy_per_momentum_density_holdout_splits.csv"
    ...
    ax.set_title("Blackbody ratio holdout: s/g ∝ T^-1")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_entropy_per_momentum_density_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_entropy_per_momentum_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "s/g is positive for photon gas; model fit is applied to abs(s/g) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for s/g (radiation entropy density per momentum density) based on blackbody identities.",
