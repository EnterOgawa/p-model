"""
目的: 黒体 holdout 監査の pressure 指標図を再生成する。
...
def main() -> None:
    ...
    pressure_coeff = radiation_constant_a / 3.0

    temperature_grid_k = np.logspace(0.0, 4.0, 80)  # 1..1e4 K
    pressure_pa = pressure_coeff * (temperature_grid_k**4)

    pressure_sigma = np.maximum(1e-12, 1e-6 * pressure_pa)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, pressure_pa, train_idx)
        pressure_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=pressure_pa, y_pred=pressure_pred, sigma=pressure_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=pressure_pa, y_pred=pressure_pred, sigma=pressure_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, pressure_pa, train_idx, exponent=4.0)
        pressure_pred_fixed = coeff_fixed * (temperature_grid_k**4.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=pressure_pa,
            y_pred=pressure_pred_fixed,
    ...
    out_csv = out_dir / "thermo_blackbody_pressure_holdout_splits.csv"
    ...
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody radiation pressure holdout: P_rad = (a/3) T^4")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_pressure_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_pressure_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for blackbody radiation pressure P_rad = (a/3) T^4.",
