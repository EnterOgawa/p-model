...
def main() -> None:
    ...
    ratio_energy_flux = energy_density / np.maximum(1e-300, flux_density)
    ratio_sigma = np.maximum(1e-12, 1e-6 * ratio_energy_flux)
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw(temperature_grid_k, ratio_energy_flux, train_idx)
        ratio_pred = (10 ** log10_coeff) * (temperature_grid_k**exponent)
        m1_train = _metrics_for_idx(
            idx=train_idx, y_obs=ratio_energy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=True
        )
        m1_test = _metrics_for_idx(
            idx=test_idx, y_obs=ratio_energy_flux, y_pred=ratio_pred, sigma=ratio_sigma, param_count=2, is_train=False
        )
        ...
        coeff_fixed = _fit_fixed_exponent(temperature_grid_k, ratio_energy_flux, train_idx, exponent=0.0)
        ratio_pred_fixed = coeff_fixed * (temperature_grid_k**0.0)
        m2_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_energy_flux,
    ...
    out_csv = out_dir / "thermo_blackbody_energy_flux_ratio_holdout_splits.csv"
    ...
    ax.set_title("Blackbody ratio holdout: u/M = 4/c")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_energy_flux_ratio_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_energy_flux_ratio_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Analytic holdout for u/M = 4/c based on blackbody energy density and flux.",
