"""
目的: 黒体 holdout 監査の photon density per momentum density 指標図を再生成する。
...
def main() -> None:
    ...
    momentum_density_coeff = radiation_constant_a / speed_of_light
    ...
    momentum_density = momentum_density_coeff * (temperature_grid_k**4)

    ratio_photon_density_per_momentum_density = photon_density / np.maximum(1e-300, momentum_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_photon_density_per_momentum_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_photon_density_per_momentum_density, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_photon_density_per_momentum_density,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(
            temperature_grid_k,
            ratio_photon_density_per_momentum_density,
            train_idx,
            exponent=-1.0,
        )
        ...
        models["fixed_exponent_minus1_abs_fit"] = {
    ...
    out_csv = out_dir / "thermo_blackbody_photon_density_per_momentum_density_holdout_splits.csv"
    ...
    for split_result in split_results:
        ...
        model_fixed = models.get("fixed_exponent_minus1_abs_fit") if isinstance(models, dict) else None
    ...
    ax.bar(
        ...
        color="#ff7f0e",
        alpha=0.85,
        label="fixed exponent -1 abs fit (1p)",
    )
    ax.axhline(3.0, color="black", linewidth=1.0, linestyle="--", alpha=0.7)
    ax.set_xticks(bar_positions)
    apply_blackbody_holdout_axes_text(ax, categories=categories, ylabel="test max abs(z)", title="Blackbody ratio holdout: n/g ∝ T^-1")
    ...
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_photon_density_per_momentum_density_holdout_splits.png"
    out_pdf = out_png.with_suffix(".pdf")
    fig.savefig(out_png, bbox_inches="tight")
    fig.savefig(out_pdf, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_photon_density_per_momentum_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "constants": {
                    ...
                    "momentum_density_coeff": momentum_density_coeff,
                },
                "assumptions": {
                    "sign_convention": "n/g is positive for photon gas; model fit is applied to abs(n/g) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for n/g (photon density per momentum density) based on blackbody identities.",
