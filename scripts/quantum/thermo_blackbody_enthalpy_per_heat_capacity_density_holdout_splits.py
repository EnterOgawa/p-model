...
def main() -> None:
    ...
    ratio_enthalpy_per_heat_capacity_density = enthalpy_density / np.maximum(1e-300, heat_capacity_density)
    ratio_sigma = np.maximum(1e-30, 1e-6 * np.abs(ratio_enthalpy_per_heat_capacity_density))
    ...
    for split in splits:
        ...
        log10_coeff, exponent = _fit_log_powerlaw_abs(temperature_grid_k, ratio_enthalpy_per_heat_capacity_density, train_idx)
        ratio_pred = np.abs((10**log10_coeff) * (temperature_grid_k**exponent))
        m1_train = _metrics_for_idx(
            idx=train_idx,
            y_obs=ratio_enthalpy_per_heat_capacity_density,
            y_pred=ratio_pred,
            sigma=ratio_sigma,
            param_count=2,
            is_train=True,
        ...
        coeff_fixed_abs = _fit_fixed_exponent_abs(
            temperature_grid_k,
            ratio_enthalpy_per_heat_capacity_density,
            train_idx,
            exponent=1.0,
        )
    ...
    out_csv = out_dir / "thermo_blackbody_enthalpy_per_heat_capacity_density_holdout_splits.csv"
    ...
    ax.set_title("Blackbody ratio holdout: h/cV ∝ T")
    ax.grid(axis="y", linestyle=":", alpha=0.35)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out_png = out_dir / "thermo_blackbody_enthalpy_per_heat_capacity_density_holdout_splits.png"
    fig.savefig(out_png, bbox_inches="tight")
    plt.close(fig)

    out_metrics = out_dir / "thermo_blackbody_enthalpy_per_heat_capacity_density_holdout_splits_metrics.json"
        ...
        json.dumps(
            {
                ...
                "assumptions": {
                    "sign_convention": "h/cV is positive for photon gas; model fit is applied to abs(h/cV) and sign=+1 is fixed.",
                ...
                "notes": [
                    "Analytic holdout for h/cV (enthalpy density per heat-capacity density) based on blackbody identities.",
