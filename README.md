...
2. **「宇宙の物差し」自体の波打ち（ゼロ・フィッティング）**
   既存の宇宙論（$\Lambda$CDM等）は、空間自体が膨張していると考え、ダークエネルギー等のパラメータをフィッティングしてきました。時間波の概念は「宇宙を測る物差し（時間や空間の基準）そのものが波打っている」と考えます。これにより、パラメータ操作を一切行わずに、BAO（バリオン音響振動）などの一次観測データに自然な説明を与えます。
