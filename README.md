...
### 📄 論文PDFのコンパイル環境について (LaTeX Environment)
本リポジトリの論文ソースファイル（`.tex`）は、日本語フォントの適切な描画とモダンな組版パッケージを利用するため、**LuaLaTeX** を標準のコンパイルエンジンとして採用しています。

ご自身のローカル環境でPDFをビルド（生成）する場合は、`lualatex` コマンドが実行可能な完全なTeXディストリビューション（**TeX studio** / **TeX Live** / **MacTeX** のフルインストール）がシステムに導入されていることを確認してください。

---

### 🛠️ システム要件: LuaLaTeX環境
P-modelの論文PDFをソースコードから完全に再現（ビルド）するには、**LuaLaTeX** エンジンが必要です。

本リポジトリに同梱されている検証・ビルド用スクリプト（例: `paper_build.py`）は、内部で `lualatex` コマンドを呼び出してPDFを生成します。実行前に以下の環境が整っているか確認してください。
* `lualatex` が実行可能なTeX環境（**TeX Live**, **MacTeX** 等）
* 日本語組版に必要な標準パッケージ群（`luatexja` 等）がインストールされていること（フルインストールを推奨します）
